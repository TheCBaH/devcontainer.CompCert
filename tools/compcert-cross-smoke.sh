...
# Distinct exit code so the caller can tell "not installed on this host"
...
run_target() {
  ...
  # 32-bit hosts (i386, armhf) ship no 64-bit-guest qemu-user binary - a
  # 32-bit process can't address a 64-bit guest. Check before building:
  # CompCert's own Rocq build can also crash outright for such a target
  # there, well before reaching the QEMU step this is really testing.
  if ! command -v "$QEMU_BIN" >/dev/null 2>&1; then
    SkipTarget "$QEMU_BIN not installed (no $TARGET user-mode emulator for this host architecture)"
  fi
