...
| S4 Difficult forms | implementing | GEN-02 through GEN-04 done. GEN-05 now promotes bare RISC-V scalar F/D arithmetic under RV32IMF(D)/RV64IMF(D), Zba's full scale-and-add family - `sh1add`/`sh2add`/`sh3add` under RV32IM_Zba/RV64IM_Zba plus the RV64-only `sh1add.uw`/`sh2add.uw`/`sh3add.uw` word-operand siblings - Zbb `min`/`minu`/`max`/`maxu` under RV32IM_Zbb/RV64IM_Zbb, Zbb's five-way import-duplicated `andn`/`orn`/`xnor`/`rol`/`ror` now that `Isa_norm_riscv` builds a real `Req_any` for them, Zbb's population-count/sign-extend/byte family - `clz`/`ctz`/`cpop`/`sext.b`/`sext.h`/`orc.b` (XLEN-independent) plus the RV64-only `clzw`/`ctzw`/`cpopw` word-operand siblings - via a new two-GPR-operand (`rd`, `rs1`, no immediate) normalization/encoder shape, and Zbkb's `brev8` (the same two-GPR shape, a four-way `Req_any` over `rv_zbkb`/`rv_zk`/`rv_zkn`/`rv_zks`, identical mnemonic/encoding on both profiles), and `rev8` (the same shape, an XLEN-dependent funct12, a per-profile five-way `Req_any`, and riscv-opcodes' own `rev8`/`rev8.rv32` native-name split unified into one rendered mnemonic and `form_id`, since real GNU as never accepts the `rev8.rv32` spelling) - closing every named blocker in this population-count/sign-extend/byte-reverse family. GEN-05 also now admits Zbkb's `pack`/`packh` (three-GPR R-type, four-way `Req_any`, identical on both profiles), the RV64-only `packw`, and the RV32-only `zip`/`unzip` (two-GPR unary), reusing the existing R-type/unary shapes and `Req_any` machinery unchanged, plus the RV64-only `rolw`/`rorw` (`rol`/`ror`'s word-operand siblings, the identical three-GPR R-type shape reusing `rev8`'s own five-way rv64-prefixed `Req_any` group verbatim), and `rori`/`rori.rv32`/`roriw` (Zbb's first rotate-*immediate* shape - a genuine two-GPR-plus-unsigned-shamt-immediate form, new `shamt_gpr_form` normalization plus `i_desc`'s existing `funct_hi`/`shamt_bits` XLEN-conditional encoder machinery already used by `srai`, `rori`/`rori.rv32` sharing one canonical rendered mnemonic via the same profile-specific native-name split `rev8`/`rev8.rv32` established). GEN-05 now also admits `zext.h`/`zext.h.rv32` (Zbb's zero-extend-halfword pseudo, reusing `unary_gpr_form` and the same profile-specific native-name split verbatim, but with a plain per-profile `Req_all` since - unlike rev8/rori - neither record is import-duplicated), closing that named follow-up. GEN-05 now also admits Zbc's `clmul`/`clmulh` (carry-less multiply, the same plain three-GPR R-type shape as `andn`/`orn`/`xnor`/`rol`/`ror`, a five-way `Req_any` over `rv_zbc`/`rv_zbkc`/`rv_zk`/`rv_zkn`/`rv_zks`, identical mnemonic/encoding on both profiles); and Zbkx's `xperm4`/`xperm8` (crossbar permute, the same three-GPR R-type shape, a four-way `Req_any` over `rv_zbkx`/`rv_zk`/`rv_zkn`/`rv_zks` - no separate non-K sibling extension the way clmul/clmulh have rv_zbc alongside rv_zbkc), fully closing that mnemonic pair; and Zknh's `sha256sum0`/`sha256sum1`/`sha256sig0`/`sha256sig1` (SHA-256 message-schedule helpers, the same two-GPR unary shape as `clz`/`ctz`/`cpop`, a three-way `Req_any` over `rv_zknh`/`rv_zk`/`rv_zkn` - no `rv_zks`), fully closing that mnemonic quartet; and Zknh's RV64-only `sha512sum0`/`sha512sum1`/`sha512sig0`/`sha512sig1` (SHA-512's own message-schedule helpers, the same two-GPR unary shape, a three-way `Req_any` over `rv64_zknh`/`rv64_zk`/`rv64_zkn`), closing every Zknh mnemonic that fits this unary shape; and Zknh's RV32-only `sha512sum0r`/`sum1r`/`sig0l`/`sig1l`/`sig0h`/`sig1h` (SHA-512's own 32-bit-word-pair-split helpers, a genuinely different plain three-GPR R-type shape reusing `r_type_gpr_form`/`r_desc`, a three-way `Req_any` over `rv32_zknh`/`rv32_zk`/`rv32_zkn`), fully closing every Zknh SHA-256/SHA-512 mnemonic on both shapes and both profiles; and RISC-V AES-64's plain-shape round functions `aes64ds`/`aes64dsm`/`aes64es`/`aes64esm`/`aes64ks2`/`aes64im` (RV64-only, reusing the existing three-GPR R-type and two-GPR unary shapes across three distinct `Req_any` groups - `rv64_zknd`-rooted, a disjoint `rv64_zkne`-rooted, and `aes64ks2`'s own four-way group imported by both), fully closing every AES/SHA mnemonic that fits an already-built shape; and RV64's `aes64ks1i` (AES-64's first key-schedule helper, a genuinely new two-GPR-plus-narrow-immediate shape - `rnum`'s valid range 0-10 is narrower than its 4-bit field's 0-15 capacity, needing its own explicitly-validated match arm rather than the shared `i_desc`/`shamt_bits` path, plus a generalized `shamt_gpr_form` accepting riscv-opcodes' own "rnum" field name instead of `rori`/`roriw`'s "shamt" one), sharing `aes64ks2`'s own four-way `Req_any` group; and RV32's `aes32dsi`/`aes32dsmi`/`aes32esi`/`aes32esmi` (AES-32's own round functions, a fourth distinct operand shape - three-GPR-plus-2-bit-`bs`-immediate, `bs` using its full field range unlike `rnum` - reusing `Lowered.R`/`word_r` unchanged via a composed `funct7`, plus a new `r_type_imm_gpr_form` normalization function, two `Req_any` groups mirroring `aes64ds`/`dsm`'s and `aes64es`/`esm`'s own), closing the entire named Zk/Zkn/Zks AES/SHA scope surveyed this session across all four operand shapes built (two-GPR unary, three-GPR plain, two-GPR-plus-narrow-immediate, three-GPR-plus-narrow-immediate); and Zicsr's `csrrw`/`csrrs`/`csrrc`/`csrrwi`/`csrrsi`/`csrrci` (self-selected from a fresh survey after the AES/SHA arc closed - top-unhandled names are now dominated by vector-crypto mnemonics needing a whole new register class, so CSR, the smallest bounded remaining family, was picked instead; XLEN-independent, the session's first family needing no `Req_any` at all; `csrrw`/`csrrs`/`csrrc` reorder GAS's own `[rd, csr, rs1]` text order against riscv-opcodes' `[rd, rs1, csr]` field order and encode `csr`'s unsigned 12-bit address via its own signed two's-complement equivalent through the unchanged `word_i` path; `csrrwi`/`csrrsi`/`csrrci` reuse the `rs1` bit position to carry a real 5-bit `zimm5` immediate with no register operand there at all), and Zicsr's own 7 GAS pseudo-op aliases `csrr`/`csrw`/`csrs`/`csrc`/`csrwi`/`csrsi`/`csrci` (each an `rd`-or-`rs1`-omitting shorthand fixing the omitted register to literal 0, three new operand shapes - `csrr rd, csr`, `csrw`/`csrs`/`csrc csr, rs1`, `csrwi`/`csrsi`/`csrci csr, zimm5` - reusing the base forms' `csr`-address two's-complement encoding via a newly factored-out shared `signed_csr` helper), fully closing every Zicsr mnemonic in the checked-in export. GEN-05 now also admits Zaamo's full atomic-memory-operation family - `amoswap`/`amoadd`/`amoxor`/`amoand`/`amoor`/`amomin`/`amomax`/`amominu`/`amomaxu`/`sc` on both `.w` (both profiles, `rv_a`) and `.d` (RV64-only, `rv64_a`), plus `lr.w`/`lr.d` (the family's only two-operand member) - the first GEN-05 family using a real GAS memory group (`(rs1)`) as an operand rather than a plain register or immediate, via two new shapes (`amo_form`/`amo3_desc` for the three-GPR-plus-memory mnemonics, `lr_form`/`lr_desc` for the two-operand one); the `.aq`/`.rl`/`.aqrl` mnemonic-suffix decorators every one of these 22 records also supports are explicitly out of scope (GAS's bare canonical spelling only, flagged via a diagnostic, not silently dropped), fully closing every Zaamo mnemonic under that canonical spelling. GEN-05 now also admits F/D's `flw`/`fld`/`fsw`/`fsd` (floating-point loads/stores, already fully implemented by the encoder's pre-existing `f_load_desc`/`f_store_desc` path, so this closes only the normalization/corpus/admission side via two new shapes `f_load_form`/`f_store_form`), closing the cheapest remaining FP increment. GEN-05 now also admits the general two-distinct-register form of `fsgnj.s`/`fsgnjn.s`/`fsgnjx.s`/`fsgnj.d`/`fsgnjn.d`/`fsgnjx.d` (the encoder previously only implemented the `rs1 = rs2` pseudo-aliases `fneg.s`/`fneg.d`/`fmv.d`; this slice adds a new `f_sgnj3_desc` encoder table/lowering arm and inverts the decode-side alias/general priority so both forms round-trip correctly, plus a new `f_sgnj_form` normalization shape), closing that named encoder gap. GEN-05 now also admits `fmin.s`/`fmax.s`/`fmin.d`/`fmax.d` (the same fixed-funct7-per-precision, funct3-selects-operation shape as `fsgnj`/`fsgnjn`/`fsgnjx`, reusing the identical `f_sgnj3_desc`/`f_sgnj_form`-style pattern via new `f_minmax_desc`/`f_minmax_form`, no alias collision to resolve this time), closing that pair. GEN-05 now also admits `fsqrt.s`/`fsqrt.d` (the arithmetic family's own implicit-dynamic-rounding shape minus a second FP operand, new `f_sqrt_desc`/`f_sqrt_form`) and `fclass.s`/`fclass.d` (the plain-GPR-result shape `fmv.x.d` established, kept in its own new `f_class_desc`/`f_class_form` rather than the shared `f_to_i_desc` table - doing so was found, and verified against real GNU as, to reproduce a pre-existing latent gap where that table's rounding-mode-override lowering arm wrongly accepts `fmv.x.d a0, fa1, rtz`, an illegal spelling real GNU as rejects, silently colliding with `fclass.d`'s own real encoding; not fixed here, flagged as a follow-up), closing that pair too. GEN-05 now also admits the fused multiply-add family `fmadd.s`/`fmsub.s`/`fnmsub.s`/`fnmadd.s`/`fmadd.d`/`fmsub.d`/`fnmsub.d`/`fnmadd.d` (RISC-V's only R4-type instructions - a genuinely new `Lowered.R4`/`word_r4` codec path, since the top 7 bits split into a 5-bit `rs3` and 2-bit `fmt` rather than one fixed `funct7`, and each mnemonic gets its own base opcode `0x43`/`0x47`/`0x4b`/`0x4f` instead of sharing OP-FP's `0x53`; new `f_fma_form` normalization, `f_arith_form`'s own implicit-dynamic-rounding shape plus a fourth FPR operand), closing that family. GEN-05 now also admits the comparison family `feq.s`/`fle.s`/`flt.s`/`feq.d`/`fle.d`/`flt.d` (a fresh survey found the encoder already had `flt.s`/`feq.d`/`fle.d`/`flt.d` support from an earlier pass with no normalization/corpus/admission ever built for them, plus `feq.s`/`fle.s` missing from the encoder entirely; the two missing entries were added to the pre-existing `f_cmp_desc` table with no new lowering arm needed, plus a new `f_cmp_form` normalization shape - `rd` a GPR, `rs1`/`rs2` FPRs, no `rm`), closing every named scalar FP comparison. GEN-05 now also admits `fmv.x.w`/`fmv.w.x` (the single-precision bit-for-bit moves `fmv.x.d` already established the shape for; `fmv.x.w` added to a new, separate `f_mv_x_w_desc` table rather than `f_to_i_desc` - the same latent-bug-avoidance reasoning `fclass.s`/`fclass.d` already used - while `fmv.w.x` was added directly to `i_to_f_desc`, which has no such bug to avoid; two new normalization shapes `f_mv_x_w_form`/`f_mv_w_x_form`), closing that pair. GEN-05 now also admits the `w`/`s` conversions `fcvt.w.s`/`fcvt.wu.s`/`fcvt.s.w`/`fcvt.s.wu` (the last named scalar-FP item; unlike every recent slice, needed two genuinely new normalization shapes, `f_cvt_w_s_form`/`f_cvt_s_w_form` - mixed-register-class, implicit-dynamic-rounding - since `fcvt.s.w`/`fcvt.s.l` and the double-precision conversions already had encoder support from an earlier, pre-isa-consumption pass but had never been wired into normalization/admission/corpus at all; `fcvt.s.wu` was added to that same pre-existing `i_to_f_desc` table, `fcvt.w.s`/`fcvt.wu.s` added directly to `f_to_i_desc` with no bug-avoidance split needed, since their own funct3 genuinely is the dynamic rounding mode), fully closing every scalar FP mnemonic named as open across every prior GEN-05 F/D writeup this session. GEN-05 now also admits the double-precision conversions `fcvt.w.d`/`fcvt.wu.d`/`fcvt.d.w`/`fcvt.d.wu`/`fcvt.s.d`/`fcvt.d.s` (all six already had encoder support from the same earlier pre-isa-consumption pass but had never been wired into norm/admission/corpus either; `fcvt.w.d`/`fcvt.wu.d` reuse `f_cvt_w_s_form` verbatim - `requirement_of` distinguishes riscv:d from riscv:f automatically - while `fcvt.d.w`/`fcvt.d.wu` and the float-to-float `fcvt.s.d`/`fcvt.d.s` needed two new forms, `f_cvt_d_w_form`/`f_cvt_f_f_form`, since real hardware's widening conversions - integer-to-double and single-to-double - default to an always-exact rne rounding mode rather than the family's usual dynamic default), fully closing every named F/D scalar conversion and every scalar F/D mnemonic flagged open across this session. GEN-05 now also admits the RV64-only long conversions `fcvt.l.d`/`fcvt.lu.d`/`fcvt.d.l`/`fcvt.d.lu`/`fcvt.l.s`/`fcvt.lu.s`/`fcvt.s.l`/`fcvt.s.lu` (only `fcvt.l.d`/`fcvt.s.l` already had encoder support; the other 6 needed genuinely new `Opcode.t` variants and table entries; no new normalization shape was needed - `fcvt.l.d`/`fcvt.lu.d`/`fcvt.l.s`/`fcvt.lu.s` reuse `f_cvt_w_s_mnemonics` and `fcvt.s.l`/`fcvt.s.lu`/`fcvt.d.l`/`fcvt.d.lu` reuse `f_cvt_s_w_mnemonics` verbatim, since - measured, not assumed - `fcvt.d.l`/`fcvt.d.lu` keep the family's usual dynamic-rounding default rather than `fcvt.d.w`/`fcvt.d.wu`'s always-exact one, because a 64-bit long is not always exact in a double the way a 32-bit int is; also added `rv64_d`/`rv64_f` to `feature_of_extension`, the first RISC-V FP mnemonics needing an XLEN-gated F/D feature), fully closing every named scalar RISC-V F/D conversion mnemonic in the checked-in export - no further scalar F/D conversion scope remains named. A separate, smaller, not-yet-scoped gap surfaced by this slice: `i_to_f_desc` has no explicit-rounding-mode-override lowering arm at all, so this project's own encoder cannot accept an explicit `rm` operand on any `i_to_f_desc`-mapped mnemonic even though real GNU as does; predates this slice, not a regression from it. GEN-05 now also admits `vsetvl` (V's register-register configuration-setting instruction, the deliberately cheapest entry point into the 375-record rv_v family - a plain three-GPR R-type shape reusing `r_type_gpr_form`/`r_desc` unchanged, a single non-import-duplicated `rv_v` record identical on both profiles, needing only a new `riscv:v` feature and OP-V's previously-unused major opcode `0x57`; confirmed against real GNU as: `vsetvl a0, a1, a2` -> `80c5f557` on both `riscv32-linux-gnu-as` 2.43.1 (`-march=rv32iv`) and `riscv64-linux-gnu-as` 2.44 (`-march=rv64iv`), no F/D dependency needed for this one instruction). GEN-05 now also admits `vsetvli`/`vsetivli` (V's own immediate-vtype siblings; GAS's own `e<SEW>,m<LMUL>,ta|tu,ma|mu` keyword-list spelling turned out not to need any frontend/parser change at all - each bare keyword already parses as a generic `Operand.Sym` through the existing text grammar, so the entire feature is a variable-length-operand-list lowering match plus a small keyword-to-bit-pattern classifier, reusing the *existing* `Lowered.I`/`word_i` codec unchanged for both: `vsetvli`'s bit31=0 constraint and `vsetivli`'s bits[31:30]=0b11 constraint both fall out automatically from plain signed-12-bit two's-complement arithmetic on the composed vtype value, the same way `signed_csr` already reuses this path for CSR's unsigned field; confirmed against real GNU as for every keyword, every legal subset/ordering, and both rejection cases (`vsetvli a0, a1, m1, e32` and `vsetvli a0, a1, ma, ta` both "illegal operands") before writing any encoder code). GEN-05 now also admits `vadd.vv`/`vadd.vx`/`vadd.vi` (the entry point into OP-V's real ~373-record vector-register arithmetic space, as opposed to the configuration-setting group above; introduces a genuinely new `Reg.V` register class - `v0`-`v31`, needing zero frontend/parser changes since `riscv_family.ml`'s bare-identifier resolution is already generic over `Reg.find` - and a new `Riscv_vec` entry in the normalized model's `register_class`; the word's top 7 bits split into a 6-bit funct6 and 1-bit vm mask-select exactly the way any other R-type's `funct7` already does, so all three mnemonics reuse the *existing* `Lowered.R`/`word_r` codec unchanged with `funct7 = (funct6 lsl 1) lor vm`; `vadd.vx`'s `rs1` stays a plain GPR and `vadd.vi`'s `rs1` position carries a real signed 5-bit immediate instead of a register, both confirmed against real GNU as including the optional trailing `, v0.t` mask operand and out-of-range/malformed rejections). GEN-05 now also admits `vsub.vv`/`vsub.vx`, `vrsub.vx`/`vrsub.vi`, and the full `vand`/`vor`/`vxor` `.vv`/`.vx`/`.vi` triples (13 mnemonics, all sharing `vadd`'s exact OPIVV/OPIVX/OPIVI shape and `funct7 = (funct6 lsl 1) lor vm` composition, differing only in the funct6 constant and, for `vsub`/`vrsub`, which of the three shapes exist at all - `vsub` has no `.vi` sibling and `vrsub` has no `.vv` sibling, matching riscv-opcodes' own export and real GNU as's "unrecognized opcode" rejection of both); this slice generalized `vadd`'s own six bespoke per-mnemonic lowering arms into three funct6-table-driven generic arms (and its three per-mnemonic normalization/entry-builder functions into `~mnemonic`-parameterized ones) that `vadd` and all five new families now share, confirmed behavior-preserving for `vadd` itself. GEN-05 now also admits the shift family `vsll`/`vsrl`/`vsra` (full `.vv`/`.vx`/`.vi` triples, the first `.vi` mnemonics using riscv-opcodes' own UNSIGNED `zimm5` field (0..31) rather than every other admitted `.vi` mnemonic's SIGNED `simm5` - a new `opivi_unsigned` predicate and `imm_name`/`signed` parameters on `opivi_form`/`opivi_entry`, not a new shape), the min/max family `vminu`/`vmin`/`vmaxu`/`vmax` (`.vv`/`.vx` only, a pure funct6-table extension), and the multiply family `vmul`/`vmulh`/`vmulhu`/`vmulhsu` (OP-V's second major functional-unit group, OPMVV/OPMVX - funct3 2/6 rather than OPIVV/OPIVX's 0/4, two new funct6 tables and lowering arms but the identical operand shape, dispatched straight to the existing `opivv_form`/`opivx_form`) - 25 mnemonics total, each a single non-import-duplicated rv_v record identical on both profiles. GEN-05 now also admits the divide/remainder family `vdivu`/`vdiv`/`vremu`/`vrem` (`.vv`/`.vx` only, the same OPMVV/OPMVX shape as `vmul`/etc., 8 mnemonics), fixing a gap along the way where `Isa_family_admission`'s separate `promoted_case` allow-list had not been updated in step with `Isa_gen_difficult.all`, and the saturating add/subtract family `vsaddu`/`vsadd`/`vssubu`/`vssub` (full `.vv`/`.vx`/`.vi` triples for `vsaddu`/`vsadd`, `.vv`/`.vx` only for `vssubu`/`vssub`, the same OPIVV/OPIVX/OPIVI shape as `vadd`/etc., 10 mnemonics), with `promoted_case` kept in step this time, and the averaging add/subtract family `vaadd`/`vaaddu`/`vasub`/`vasubu` (`.vv`/`.vx` only, the same OPMVV/OPMVX shape as `vmul`/`vdivu`/etc., 8 mnemonics), and the narrowing shift/clip family `vnsrl`/`vnsra`/`vnclipu`/`vnclip` (full `.wv`/`.wx`/`.wi` triples, the same OPIVV/OPIVX/OPIVI shape as `vadd`/etc. despite the `.w*` suffix denoting a semantically wide `vs2`, 12 mnemonics), and the scaling shift-right pair `vssrl`/`vssra` (full `.vv`/`.vx`/`.vi` triples, the same shape as `vsll`/`vsrl`/`vsra`, 6 mnemonics), and the gather/permute family `vrgather` (full `.vv`/`.vx`/`.vi` triple) plus `vrgatherei16.vv` (`.vv`-only sibling, 4 mnemonics total), and the widening add/subtract family `vwaddu`/`vwadd`/`vwsubu`/`vwsub` (`.vv`/`.vx`/`.wv`/`.wx`, no `.vi` sibling, 16 mnemonics), and the widening multiply family `vwmulu`/`vwmulsu`/`vwmul` (`.vv`/`.vx` only, 6 mnemonics; the sibling widening multiply-*accumulate* `vwmacc*` family was investigated and found to need a genuinely different, reordered operand shape - deliberately deferred, see GEN-06 candidate below), and the sign-/zero-extend family `vsext`/`vzext` (`.vf2`/`.vf4`/`.vf8`, 6 mnemonics; the first GEN-05 vector slice needing a genuinely new two-vector-register shape with no third operand), and the mask-register logical family `vmand`/`vmandn`/`vmor`/`vmxor`/`vmorn`/`vmnand`/`vmnor`/`vmxnor` (`.mm`, 8 mnemonics; the same all-vector-register `rd, rs2, rs1` shape as `vadd.vv`/etc. but with `vm` architecturally fixed at 1 - no masked sibling exists, confirmed against real GNU as - so this project keeps its funct6 lookup in its own table and gives it a dedicated normalization function and lowering arm rather than folding it into the existing OPMVV plumbing), and the vector-reduction family `vredsum`/`vredand`/`vredor`/`vredxor`/`vredminu`/`vredmin`/`vredmaxu`/`vredmax.vs` (8 mnemonics, the same `rd, rs2, rs1` shape as `vadd`/etc. but with a real, selectable mask, needing only funct6-table extensions and mnemonic-list membership - no new shape) plus `vwredsumu`/`vwredsum.vs` (2 mnemonics, the widening-sum reduction pair, sharing OPIVV's funct3 space rather than OPMVV's despite being widening - confirmed against real GNU as before adding the table entry), and the mask-writing comparison family `vmseq`/`vmsne`/`vmsltu`/`vmslt`/`vmsleu`/`vmsle`/`vmsgtu`/`vmsgt` (20 mnemonics, the full OPIVV/OPIVX/OPIVI shape minus `.vv` for `vmsgtu`/`vmsgt` and `.vi` for `vmsltu`/`vmslt` - real GNU as accepts `vmsgt(u).vv` only as a pseudo-instruction reversing `vmslt(u).vv`'s own operands, a genuinely different alias-expansion feature deliberately not admitted here - again needing only funct6-table extensions and mnemonic-list membership, no new shape), and the slide family `vslideup`/`vslidedown` (OPIVX/OPIVI, no `.vv` sibling) plus `vslide1up`/`vslide1down` (OPMVX, no `.vi` sibling - inserting one element needs a real scalar, not a 5-bit immediate) - 6 mnemonics total, the `.vi` immediate UNSIGNED `zimm5` like the shift-family shapes - again a pure funct6-table extension, no new shape, and the multiply-accumulate family `vmacc`/`vnmsac`/`vmadd`/`vnmsub` plus the widening `vwmaccu`/`vwmacc`/`vwmaccsu`/`vwmaccus` (15 mnemonics, OPMVV/OPMVX, finally claiming the reordered-operand-shape candidate flagged as deliberately deferred since the widening-multiply writeup: real GNU as's text order is `vd, vs1-or-rs1, vs2`, the reverse of every other OPMVV/OPMVX mnemonic's `vd, vs2, vs1-or-rs1` - confirmed by decoding the assembled word's own vs1/vs2 field bits, not just accepted/rejected status - needing two new normalization form functions (`opmacc_vv_form`/`opmacc_vx_form`) and two new funct6 tables/lowering-arm pairs kept deliberately separate from `opmvv_funct6`/`opmvx_funct6` because of that shape difference; `vwmaccus` has no `.vv` sibling), and the permute family `vid.v`/`viota.m`/`vcompress.vm` (3 mnemonics, each surveyed and deliberately deferred in the slide-family writeup above as needing a genuinely new shape, but cheaper than expected once measured: `viota.m` reuses `vext_form`'s existing "rd, vs2" shape verbatim after generalizing the encoder's `opmvv_unary_const` table to carry funct6 alongside its fixed constant, `vcompress.vm` reuses `mm_form`'s existing fixed-vm-at-1 "rd, vs2, vs1" shape verbatim via one new `mm_funct6` entry, and only `vid.v` needed genuinely new code - a new single-operand `vid_form` normalization shape and dedicated encoder arms, the project's first OP-V mnemonic with no source register at all). Every mnemonic named as a deferred candidate across the entire OP-V arithmetic/permute survey to date is now closed, and the mask-scalar family `vcpop.m`/`vfirst.m`/`vmsbf.m`/`vmsif.m`/`vmsof.m` (5 mnemonics, self-selected after a fresh survey of the remaining 193 unclaimed rv_v records via `compcert_tools isa-inventory family-admission`: `vmsbf.m`/`vmsif.m`/`vmsof.m` reuse `vext_form`'s exact shape verbatim, funct6 0x14 like `vid.v`/`viota.m` but disambiguated by their own fixed rs1-position constants, and `vcpop.m`/`vfirst.m` need only a GPR-destination variant of that same shape via a new `opmvv_gpr_unary_const` table/arm pair, not a new shape family). GEN-05 now also admits the add-with-carry/subtract-with-borrow family `vadc`/`vmadc`/`vsbc`/`vmsbc` (15 mnemonics, confirming the deferred "genuinely different masking discipline" prediction: `vm` is baked as a per-opcode fixed constant rather than a toggleable `, v0.t` suffix, and the "m"-suffixed forms carry a mandatory literal `v0` 4th operand modeled as a real `vcarry` operand rather than left to the encoder - six new dedicated normalization forms and six new funct6-table/lowering-arm pairs, since `opivv_form`/`opivx_form`/`opivi_form` would both omit that operand and falsely claim an optional mask). GEN-05 now also admits `vmerge.vvm`/`.vxm`/`.vim` (3 mnemonics, confirming the predicted reuse: identical mandatory-`v0` shape to the carry family above, needing only three new funct6-table entries with zero new lowering code). GEN-05 now also admits the `vmv` scalar-move and whole-register-move family (9 mnemonics: `vmv.x.s`/`vmv.s.x` GPR-vector move pair, `vmv.v.v`/`.v.x`/`.v.i` unconditional-move family with no `vs2` operand at all, and `vmv1r.v`/`2r.v`/`4r.v`/`8r.v` whole-register-group move - three genuinely distinct new two-operand shapes, none with a masked sibling; confirmed real GNU as does not enforce the whole-register-group's own register-alignment requirement at assembly time, so this project's encoder does not either). GEN-05 now also admits the saturating fixed-point multiply pair `vsmul.vv`/`vsmul.vx` (2 mnemonics, the cheapest GEN-05 slice this session - despite the "multiply" name, real GNU as places it in OPIVV/OPIVX's own funct3 space rather than the OPMVV/OPMVX space every other multiply family uses, so it reuses the *existing* `opivv_funct6`/`opivx_funct6` tables and lowering arms verbatim with just two new entries). Every scalar-shaped OP-V arithmetic/permute/move mnemonic surveyed to date is now closed. The remaining vector scope is entirely the two large, structurally distinct groups repeatedly deferred across this whole GEN-05 vector arc: the entire floating-point vector family `vf*` (~90 mnemonics, needs an FPR-as-vector-operand shape not yet built) and every vector load/store mnemonic (needing segment/strided/indexed memory-addressing modes this project's encoder does not have at all) - plus every rv_zv* vector-crypto extension; GEN-06 has no single named next item, but for the first time in this session the two remaining candidates are both genuinely large (tens of mnemonics, new infrastructure each), not small bounded slices. GEN-05 has since opened the `vf*` family with its entry point `vfadd.vv`/`vfadd.vf` (OPFVV/OPFVF, funct6 0x00), building the first FPR-typed OP-V scalar-broadcast shape (`opfvf_form`) and confirming real GNU as enforces no F/D dependency on OP-V floating mnemonics at assembly time - the governing precedent for the ~88 remaining `vf*` mnemonics, which still need their own survey and slicing. GEN-05 now also admits `vfsub.vv`/`vfsub.vf`/`vfrsub.vf` (funct6 0x02/0x27, a pure `opfvv_funct6`/`opfvf_funct6` table extension with zero new normalization or lowering code - `vfrsub` has no `.vv` sibling), closing that pair-plus-reverse triple. GEN-05 now also admits `vfmul.vv`/`vfmul.vf`/`vfdiv.vv`/`vfdiv.vf`/`vfrdiv.vf` (funct6 0x24/0x20/0x21, confirming every OP-V floating arithmetic mnemonic stays in OPFVV/OPFVF regardless of operation, unlike integer multiply/divide's own OPMVV/OPMVX space - another pure funct6-table extension, `vfrdiv` has no `.vv` sibling and `vfmul` has no `.vi` sibling). GEN-05 now also admits `vfmin.vv`/`vfmin.vf`/`vfmax.vv`/`vfmax.vf` (funct6 0x04/0x06, full `.vv`/`.vf` pairs with no `.vi` sibling for either - yet another pure funct6-table extension). GEN-05 now also admits the sign-injection triple `vfsgnj.vv`/`vfsgnj.vf`/`vfsgnjn.vv`/`vfsgnjn.vf`/`vfsgnjx.vv`/`vfsgnjx.vf` (funct6 0x08/0x09/0x0a, full `.vv`/`.vf` pairs with no `.vi` sibling for any - unlike scalar `fsgnj.s`/`fsgnj.d`, the vector forms have no `rs1 = rs2` pseudo-alias collision to resolve, so this too is a plain table extension). GEN-05 now also admits the floating unary family `vfsqrt.v`/`vfrsqrt7.v`/`vfrec7.v`/`vfclass.v` (funct6 0x13, `vext_form`'s exact "vd, vs2" shape disambiguated by a fixed rs1-position constant - the first `vf*` slice needing a new encoder table, `opfvv_unary_const`, since the funct6-table extensions above only cover the three-operand shape). GEN-05 now also admits the floating vector-reduction family `vfredosum.vs`/`vfredusum.vs`/`vfredmin.vs`/`vfredmax.vs` (funct6 0x03/0x01/0x05/0x07, the same `rd, rs2, rs1` all-vector shape as `vfadd.vv`/etc. - the cheapest `vf*` slice yet, fitting directly into the *existing* `opfvv_funct6` table with zero new lowering code, since these stay in OPFVV rather than needing the integer `vredsum`/etc. family's own separate `opmvv_funct6` table). GEN-05 now also admits the mask-writing floating comparison family `vmfeq`/`vmfle`/`vmflt`/`vmfne` `.vv`/`.vf` pairs plus `vmfgt.vf`/`vmfge.vf` (funct6 0x18/0x19/0x1b/0x1c/0x1d/0x1f, the same shape as `vfadd.vv`/`.vf` - `vmfgt`/`vmfge` have no `.vv` sibling, matching the integer `vmsgt`/`vmsgtu` alias-expansion precedent - another pure funct6-table extension). GEN-05 now also admits `vfmv.f.s`/`vfmv.s.f`/`vfmv.v.f` (funct6 0x10/0x10/0x17, the FPR-typed mirror of `vmv.x.s`/`vmv.s.x`/`vmv.v.x` - the first `vf*` slice needing genuinely new normalization forms, `vfmv_f_s_form`/`vfmv_s_f_form`, plus a generalized `vmv_v_form` `rs1_kind` variant extended to include `` `Fpr ``). GEN-05 now also admits `vfmerge.vfm` (funct6 0x17, the FPR-typed mirror of `vmerge.vxm` - mandatory literal `v0` fourth operand, no bare non-"m" sibling and no `, v0.t` masked form, matching `vmerge.vxm`'s own precedent exactly). GEN-05 now also admits the scalar-width float<->integer conversion family `vfcvt.xu.f.v`/`vfcvt.x.f.v`/`vfcvt.f.xu.v`/`vfcvt.f.x.v`/`vfcvt.rtz.xu.f.v`/`vfcvt.rtz.x.f.v` (funct6 0x12, `vext_form`'s exact "vd, vs2" shape reusing the *existing* `opfvv_unary_const` table verbatim - zero new table or lowering-arm code, the cheapest `vf*` slice since `vfredosum`/etc.). GEN-05 now also admits the 16-mnemonic floating fused-multiply-add family `vfmadd.vv`/`.vf`, `vfnmadd.vv`/`.vf`, `vfmsub.vv`/`.vf`, `vfnmsub.vv`/`.vf`, `vfmacc.vv`/`.vf`, `vfnmacc.vv`/`.vf`, `vfmsac.vv`/`.vf`, `vfnmsac.vv`/`.vf` under OPFVV/OPFVF (the `.vv` half reusing the integer `vmacc` family's reordered `[vd, vs1, vs2]` `opmacc_vv_form`/`opmacc_vv_entries` machinery verbatim under new funct6 values 0x28-0x2f, the `.vf` half a genuinely new FPR-typed mirror `opfmacc_vf_form`/`opfmacc_vf_entries`). GEN-05 now also admits `vfslide1up.vf`/`vfslide1down.vf` (the slide family's floating single-element siblings, funct6 0x0e/0x0f under OPFVF, a zero-new-code slice reusing the *existing* `opfvf_form`/`opfvf_entries` machinery verbatim under two new funct6 table entries) - the last small bounded `vf*` slice; only the widening `vfw*`/narrowing `vfn*` families remained. GEN-05 has since closed the entire non-`rv_zvfbfmin` `vfw*`/`vfn*` scope (widening add/subtract/multiply/reduction/conversion/FMA), and has now moved into `rv_v`'s remaining 58-record load/store territory with the unit-stride `vle8.v`/`vle16.v`/`vle32.v`/`vle64.v`/`vse8.v`/`vse16.v`/`vse32.v`/`vse64.v` family (a genuinely new "vd/vs3, (base)" memory-operand shape, encoded via the existing generic R-type packer) and its mask-register `vlm.v`/`vsm.v` sibling (same shape, fixed lumop/sumop and vm permanently 1, no masked spelling) and its fault-only-first `vle{8,16,32,64}ff.v` sibling (identical load shape, fixed lumop=0x10, masked spelling accepted), and the strided `vlse8.v`/`vlse16.v`/`vlse32.v`/`vlse64.v`/`vsse8.v`/`vsse16.v`/`vsse32.v`/`vsse64.v` family (a new three-operand "vd/vs3, (base), rs2" shape, mop=0b10), and the indexed `vluxei*`/`vloxei*`/`vsuxei*`/`vsoxei*` family (16 mnemonics, the strided shape with a vector-register index, mop=0b01/0b11), and the whole-register `vl{1,2,4,8}re{8,16,32,64}.v`/`vs{1,2,4,8}r.v` family (20 mnemonics - the register count is baked into the mnemonic, so this reuses `vlm_form`/`vsm_form` with zero new normalization code) - the entire 375-record `rv_v` family is now promoted-support with zero blockers. GEN-05 has since also closed `rv_v_aliases`' remaining 11-record blocker (`vpopc.m`/`vmandnot.mm`/`vmornot.mm`/`vfredsum.vs`/`vfwredsum.vs`/`vl1r.v`/`vl2r.v`/`vl4r.v`/`vl8r.v`/`vle1.v`/`vse1.v`, riscv-opcodes' own deprecated pseudo-op spellings for already-admitted `rv_v` mnemonics, each reusing its canonical mnemonic's own normalization shape and needing only a small deprecated-spelling-to-canonical-name table in `Opcode.of_mnemonic`, no new opcode variant), so every `rv_v`/`rv_v_aliases` record is now promoted-support with zero blockers. GEN-05 has since closed every `rv_zv*` vector-crypto/bf16 extension (all twelve families, plus the `rv_zvkn`/`rv_zvks` bundle extensions) in full, and has now also closed Zbs's single-bit-manipulation family (`bclr`/`bext`/`binv`/`bset` plus their shift-amount-immediate siblings `bclri`/`bexti`/`binvi`/`bseti`) Zbc's remaining `clmulr` mnemonic, Zicond's `czero.eqz`/`czero.nez` pair, Zksh's `sm3p0`/`sm3p1` pair, and Zksed's `sm4ed`/`sm4ks` pair - see the milestone writeups above for all six. Per this file's own S4 guidance to weigh RISC-V scalar slices against x86's own unadmitted space, GEN-05 has now turned to x86: it admits the plain register-register ALU family `subl`/`andl`/`orl`/`xorl`/`adcl`/`sbbl`/`cmpl`/`testl`, generalizing `ADD_GPRv_GPRv_01`'s own `to_rm_r` opcode-selection precedent (real GNU as always selects this "low-numbered" iform for two register operands regardless of AT&T argument order) to the rest of that opcode table, on both x86-32 and x86-64, and now also admits `ADD_GPRv_MEMv`/`ADC_GPRv_MEMv`/`XOR_GPRv_MEMv` (the register<-memory ALU direction, the only three `to_r_rm` opcodes this project's encoder currently lowers with a memory source), via a new `alu_gprv_memv_form` generalizing GEN-04's own `mov_gprv_memv_form` shape to read each operand's real `rw` fact instead of hardcoding MOV's write-only role, and has now also admitted the remaining `SUB_GPRv_MEMv`/`AND_GPRv_MEMv`/`OR_GPRv_MEMv`/`SBB_GPRv_MEMv`/`CMP_GPRv_MEMv` (the first GEN-05 x86 slice needing a real encoder change: new `Opcode.to_r_rm` table entries, a generalized `Alu_r_rm` lowering arm, and a generalized `alu_r_rm_codec` entries list), closing every `to_r_rm`-reachable GPRv_MEMv ALU mnemonic in the checked-in export, and has now also admitted `OR_GPRv_IMMz`/`ADC_GPRv_IMMz`/`SBB_GPRv_IMMz`/`AND_GPRv_IMMz`/`SUB_GPRv_IMMz`/`XOR_GPRv_IMMz`/`CMP_GPRv_IMMz` (generalizing `ADD_GPRv_IMMz`'s own shape to an explicit-width mnemonic; `Sbb` needed `Opcode.to_ext`/`of_ext`'s one gap, ext 3, filled), closing the base legacy x86 ALU/MOV opcode space (register-register, register<-memory, register/immediate) essentially in full - I86 now stands at promoted-support 26/506 (x86-32) and 26/443 (x86-64), X87 at 1/142 and 1/138. GEN-05 has since admitted the same register/immediate ALU family's imm8 rung (`ADD/OR/ADC/SBB/AND/SUB/XOR/CMP_GPRv_IMMb`, opcode 0x83) and both `_MEMv_IMMb`/`_MEMv_IMMz` memory-destination directions (opcodes 0x83/0x81) of all eight mnemonics - 24 new iforms per x86 profile, needing zero encoder changes since this project's own `lower_instruction` already lowered an `Operand.Mem` ALU-immediate destination through the same `Lowered.Alu_rm_imm` codec the register destination uses - moving I86 to promoted-support 50/506 (x86-32) and 50/443 (x86-64). GEN-05 has since admitted the byte-width GRP1 family (opcode 0x80, `GPR8_IMMb`/`MEMb_IMMb`) and the accumulator-immediate family (opcode `ext<<3|4`, `AL_IMMb`) for all eight mnemonics - 24 new records per x86 profile, needing a real encoder change (a new `alu_form_byte` opcode-0x80 rung and `alu_acc_form_byte` accumulator rung, `alu_form`'s own two rungs gaining an explicit `width <> 8` guard against a latent register-number-reuse bug) since `Alu_rm_imm`/`alu_form` were previously scoped to width 32/64 only, and then the reverse `MEMv<-GPRv` ALU store direction (`ADD_MEMv_GPRv`/.../`TEST_MEMv_GPRv`, 9 records per x86 profile), which needed zero encoder table changes since `Opcode.to_rm_r` already covered it (memory and register r/m share one opcode, distinguished only by ModR/M's mod field) - only a new lowering arm and `alu_memv_gprv_form` normalizer - closing the base legacy x86 ALU/MOV opcode space in full across every addressing/width combination this project's encoder builds, and moving I86 to promoted-support 83/506 (x86-32) and 83/443 (x86-64). GEN-05 has now also opened the x86 vector/SIMD space: `ADDSD`/`SUBSD`/`MULSD`/`DIVSD_XMMsd_XMMsd` (SSE2 scalar-float register-register binops, 4 records per x86 profile), admitted through the model's first new register class since X87_st (`X86_xmm`) plus a new `xmm_binop_rr_form`, reusing `x86_family_encode.ml`'s own already-built `Sse_binop_r_rm`/`sse_binop_f2_codec` machinery (predating this ISA-consumption work) with zero encoder changes, then closed the immediate register<-memory follow-up (`ADDSD`/`SUBSD`/`MULSD`/`DIVSD_XMMsd_MEMsd`, 4 more records per x86 profile, `xmm_binop_rm_form`, again zero encoder changes) - SSE2 now stands at promoted-support 8/263 (x86-32) and 8/268 (x86-64), with the rest of SSE/SSE2 (`movsd`/`movss` and eleven other already-encoder-supported mnemonics) named as ready follow-ups and VEX/EVEX still fully unadmitted. GEN-05 has now closed the rest of the plain xmm-xmm/xmm-memory binop shape in both directions: `MULSS`/`DIVSS` (SSE, not SSE2 - `Isa_norm_xed.requirement_of` gained its own `Req_feature "x86:sse"` case, confirmed distinct from SSE2 via `provenance.extension`) and `COMISD`/`UCOMISD`/`COMISS`/`XORPD`/`PXOR`/`MOVAPD_XMMpd_XMMpd_0F28`/`CVTSD2SS`/`CVTSS2SD` (SSE2/SSE) all reuse `xmm_binop_rr_form`/`xmm_binop_rm_form` verbatim - `x86_family_encode.ml`'s existing `sse_binop_f3_codec`/`sse_binop_66_codec`/`sse_binop_none_alt` tables already fully implement every one of these ten mnemonics (`comiss` alone has no mandatory prefix), so this is pure normalization/admission wiring, zero encoder change, confirmed byte-for-byte against real GNU as for every register-register and register<-memory spelling first (`x86_64-linux-gnu-as`/`objdump`, both 64- and 32-bit mode). `MOVAPD`'s own reverse `MEMpd<-XMMpd` store direction and its redundant `_0F29` register-register iform stay unadmitted, matching this file's to_rm_r "low-numbered iform" precedent (confirmed: real GNU as selects opcode 0x28, not 0x29, for `movapd %xmm1, %xmm0`). This slice moves 20 more records per x86 profile (10 mnemonics x 2 directions) - SSE2 now stands at promoted-support 22/263 (x86-32) and 22/268 (x86-64), SSE at 6/98 (both profiles), with `movsd`/`movss` (a mixed GPR-free but load/store-shaped pair) and the two GPR-mixed conversion families `cvtsi2sd`/`cvtsi2ss`/`cvttsd2si` named as the remaining already-encoder-supported follow-ups and VEX/EVEX still fully unadmitted. GEN-05 has now admitted `movsd`/`movss` load/store (`MOVSD_XMM_XMMdq_MEMsd`/`MOVSD_XMM_MEMsd_XMMsd` and their MOVSS siblings, 4 records per x86 profile): a plain move rather than another binop - `x86_family_encode.ml`'s own comment on `Lowered.Sse_mov_r_rm`/`Sse_mov_rm_r` already notes register-register `movsd`/`movss` is unbuilt (unevidenced by the corpus) - so only the load/store direction is admitted, via a new `Isa_norm_xed.xmm_mov_form` generalizing the pre-existing `mov_gprv_memv_form`'s own `~load` direction flag to the `X86_xmm` register class instead of `X86_gpr`, confirmed byte-for-byte against real GNU as (`x86_64-linux-gnu-as`/`objdump`) before writing any normalization code. Zero encoder change. SSE2 now stands at promoted-support 26/263 (x86-32) and 26/268 (x86-64), with the GPR-mixed conversion trio `cvtsi2sd`/`cvtsi2ss`/`cvttsd2si` named as the sole remaining already-encoder-supported SSE/SSE2 follow-up and VEX/EVEX still fully unadmitted. GEN-05 has now admitted that trio (`CVTSI2SD_XMMsd_GPR32d`/`GPR64q`/`MEMd`/`MEMq` and their `CVTSI2SS` siblings, plus `CVTTSD2SI_GPR32d_XMMsd`/`GPR64q_XMMsd`/`GPR32d_MEMsd`/`GPR64q_MEMsd` - 12 records per x86 profile), the model's first mixed-register-class shape: four new `Isa_norm_xed` forms (`cvtsi2f_rr_form`/`cvtsi2f_rm_form` for `cvtsi2sd`/`cvtsi2ss`, `cvtf2i_rr_form`/`cvtf2i_rm_form` for `cvttsd2si`), reusing `x86_family_encode.ml`'s pre-existing `Lowered.Cvtsi2f_r_rm`/`Cvtf2i_r_rm` support unchanged - zero encoder change. XED reports the 32-bit and 64-bit GPR widths as separate records with `provenance.mode_restriction` "unspecified" (a GPR64 operand implies 64-bit mode by register-class fact alone, not by an applicability token `requirement_of_applicability` would see), so the 64-bit records (`GPR64q`) carry a derived `Req_mode {mode="mode64"; equals=true}` on top of `requirement_of`'s own SSE/SSE2 feature and are promoted on `X86_64` only, confirmed against real GNU as (`cvtsi2sdq %rax, %xmm0`/`cvttsd2si %xmm0, %rax` assemble only in 64-bit mode) before writing any normalization code; the six 32-bit-width records promote on both targets like every prior x86 form. This slice moves 12 more records per x86 profile (6 promoted on both targets, 6 additionally promoted on `X86_64` only) - SSE2 now stands at promoted-support 28/263 (x86-32) and 32/268 (x86-64), SSE at 10/98 (x86-32) and 12/98 (x86-64), closing the named SSE/SSE2 admission follow-ups. A fresh survey after that closure found `XORPD` was one member of x86's packed bitwise-logical family - `ANDPS`/`ANDNPS`/`ORPS`/`XORPS` (SSE, no mandatory prefix) and `ANDPD`/`ANDNPD`/`ORPD` (SSE2, `66` mandatory prefix, `XORPD`'s own siblings) - sharing `XORPD`'s exact `Sse_binop_r_rm` shape and mandatory-prefix grouping but, unlike every slice since ADDSD, genuinely absent from the encoder's `Opcode.t` and opcode tables rather than merely unadmitted; GEN-05 now admits all seven (register-register and register<-memory, 14 forms per x86 profile), adding the minimal new `Opcode.t` variants and opcode-table entries - generalizing `sse_binop_none_alt` into a table over its opcode the same way `sse_binop_alt` already is generalized over its mandatory-prefix groups - while reusing `Sse_binop_r_rm` and `xmm_binop_rr_form`/`xmm_binop_rm_form` verbatim with no new normalization shape, confirmed against real GNU as (`i686-linux-gnu-as` 2.44: `0F 54/55/56/57` for the `ps` forms, `66 0F 54/55/56` for the `pd` forms) before writing any code. SSE2 now stands at promoted-support 34/263 (x86-32) and 38/268 (x86-64), SSE at 18/98 (x86-32) and 20/98 (x86-64); only VEX/EVEX remain unadmitted in x86 vector/SIMD. A fresh survey after that closure found `MOVAPD` was itself one member of a data-movement family - `MOVAPS`/`MOVUPS` (SSE, no mandatory prefix) and `MOVUPD` (SSE2, `66` mandatory prefix) - sharing `MOVAPD`'s exact `Sse_binop_r_rm` shape (register-register plus register<-memory load direction only, `MOVAPD`'s own reverse-store precedent) but, like the `ANDPS` family before it, genuinely absent from the encoder's `Opcode.t` and opcode tables; GEN-05 now admits all three (register-register and register<-memory, 6 forms per x86 profile), adding three new `Opcode.t` variants and opcode-table entries (`MOVAPS`/`MOVUPS` into `sse_binop_none_codec`, `MOVUPD` into `sse_binop_66_codec`) while reusing `Sse_binop_r_rm` and `xmm_binop_rr_form`/`xmm_binop_rm_form` verbatim with no new normalization shape, confirmed against real GNU as (`i686-linux-gnu-as` 2.44: `movaps %xmm1,%xmm2` -> `0f 28 d1`, `movups %xmm1,%xmm2` -> `0f 10 d1`, `movupd %xmm1,%xmm2` -> `66 0f 10 d1`) before writing any code. SSE2 now stands at promoted-support 36/263 (x86-32) and 40/268 (x86-64), SSE at 22/98 (x86-32) and 24/98 (x86-64); every already-encoder-supported scalar/plain-binop-shaped SSE/SSE2 mnemonic surveyed to date is now admitted, and only VEX/EVEX remain unadmitted in x86 vector/SIMD. A fresh survey confirmed opcode bytes `0x58`/`0x59`/`0x5C`/`0x5E` - already used by the F2-prefixed `ADDSD`/`MULSD`/`SUBSD`/`DIVSD` and F3-prefixed `ADDSS`/`MULSS`/`SUBSS`/`DIVSS` - have the same no-mandatory-prefix (`ps`) and `66`-mandatory-prefix (`pd`) siblings the `ANDPS`/`MOVAPS` families' own opcode groups already had: `ADDPS`/`SUBPS`/`MULPS`/`DIVPS` (SSE) and `ADDPD`/`SUBPD`/`MULPD`/`DIVPD` (SSE2), genuinely absent from the encoder's `Opcode.t` and opcode tables; GEN-05 now admits all eight (register-register and register<-memory, 16 forms per x86 profile), adding eight new `Opcode.t` variants and opcode-table entries (`ADDPS`/`SUBPS`/`MULPS`/`DIVPS` into `sse_binop_none_codec`, `ADDPD`/`SUBPD`/`MULPD`/`DIVPD` into `sse_binop_66_codec`) while reusing `Sse_binop_r_rm` and `xmm_binop_rr_form`/`xmm_binop_rm_form` verbatim with no new normalization shape, confirmed against real GNU as (`i686-linux-gnu-as`/`x86_64-linux-gnu-as` 2.44: `addps %xmm1,%xmm0` -> `0f 58 c1`, `addpd %xmm1,%xmm0` -> `66 0f 58 c1`, and likewise for `sub`/`mul`/`div` at `0x5C`/`0x59`/`0x5E`) before writing any code. SSE2 now stands at promoted-support 44/263 (x86-32) and 48/268 (x86-64), SSE at 30/98 (x86-32) and 32/98 (x86-64); this completes the prefix square for every already-encoder-supported scalar/plain-binop-shaped SSE/SSE2 opcode group surveyed to date, and only VEX/EVEX remain unadmitted in x86 vector/SIMD. GEN-05 has now opened x86's own VEX/AVX vector-extension work in earnest (not merely legacy SSE/SSE2, which real Intel/GNU terminology never calls "vector extensions" - AVX is the actual x86 feature by that name): it admits `vaddsd`/`vsubsd`/`vmulsd`/`vdivsd` (`VEX.LIG.F2.0F.WIG` register-register form only), the deliberately cheapest entry point into x86's VEX/EVEX space the way `vsetvl` was RISC-V's own - a genuinely new `Lowered.Vex_binop_rrr` shape (`dst := src1 op src2`, three real registers, non-destructive, unlike every legacy binop's destructive `dst := dst op src`) and a new two-byte-VEX (`0xC5`) codec path (`vex_scalar_f2_rrr_alt`/`vex_scalar_f2_codec`), reusing the existing `rm_codec` for the ModR/M byte and a new `vex_binop_rrr_form` normalizer plus an `x86:avx` feature mapping. `src2` (the ModR/M r/m operand) is restricted to xmm0-7, since reaching xmm8-15 there needs REX.B's VEX counterpart, which only the not-yet-built three-byte VEX prefix (`0xC4`) carries; `dst` and `src1` (the VEX prefix's own `vvvv` field) already reach all of xmm0-15 through the two-byte prefix alone. Confirmed against real GNU as before writing any code and re-verified after: `vaddsd %xmm2,%xmm1,%xmm0` -> `c5 f3 58 c2`, `vaddsd %xmm7,%xmm3,%xmm5` -> `c5 e3 58 ef`, `vaddsd %xmm2,%xmm9,%xmm10` -> `c5 33 58 d2` (confirming `dst`/`src1` reach xmm8+ correctly via R/vvvv alone), and `vaddsd %xmm8,...` correctly rejected by a new named diagnostic (`Vex_rm_extended_register`) rather than silently misencoding. One accounting caveat surfaced by this slice, not a regression: `Isa_gen_oracle.observed_form_check`'s x86 leg only ever compares the assembled bytes' first byte against the record's plain `opcode` field, a heuristic built solely for legacy (non-VEX) encodings; since a VEX-encoded instruction's first byte is always the VEX prefix (`0xC5`/`0xC4`) rather than the opcode, this reports `DIFFERENT-FORM` for every VEX case even when, as confirmed here, our bytes agree byte-for-byte with real GNU as - `asm-isa-difficult-check` correctly still credits these as passes ("both accept and bytes agree"), but the specific per-record form-identity assertion does not yet distinguish "different but still correct" from "wrong" for VEX/EVEX; a future x86 vector slice should give the checker real VEX/EVEX-aware form identification rather than leaving this heuristic silently inapplicable. Deliberately deferred in this slice: the register<-memory sibling, YMM (VEX.L), three-byte VEX, and EVEX - each a distinct, larger future slice. That VEX-blind-spot follow-up is now fixed: `Isa_gen_oracle.observed_form_check`'s `X86_encoding` branch previously assumed byte 0 of GAS's own assembled output was always the record's plain trailing opcode byte, a heuristic that only ever held for `opcode_map = 0` legacy forms with no mandatory prefix - it already silently mismatched every mandatory-prefix legacy SSE/SSE2 record too (confirmed: 162 of 328 checked-in x86 cases already reported `different_observed_form` before this fix, including every `ADDSD`/`MULSS`/etc. case, purely a pre-existing diagnostic gap this project's own `asm-isa-difficult-check` never gated on, since pass/fail there is decided by independent GAS-vs-ours byte equality, not this per-record form-identity check). The fix is narrowly scoped to what the normalized model can state with certainty: when a record's `X86_encoding.space` is `"vex"`, the checker now inspects GAS's own leading byte to tell a two-byte (`0xC5`) from a three-byte (`0xC4`) VEX prefix and compares the opcode field against the byte that actually follows it (offset 2 or 3) instead of offset 0 - confirmed this correctly flips `VADDSD`/`VSUBSD`/`VMULSD`/`VDIVSD`'s register-register cases from `different_observed_form` to `matches_normalized_encoding` in the regenerated corpus, while every legacy-space case's check (including the still-uncorrected mandatory-prefix cases) is byte-for-byte unchanged, verified via `tools-test`'s existing `0x03 leading byte does not match ADD_GPRv_GPRv_01` regression and the full corpus regen. The wider legacy mandatory-prefix version of this same gap (`X86_encoding` has no field recording whether a mandatory `66`/`F2`/`F3` prefix precedes the opcode_map escape bytes) remains open and unrelated to VEX - a separate future task, not addressed here. GEN-05 has now also closed the first of the two deliberately deferred VEX follow-ups named above: the register<-memory sibling of `vaddsd`/`vsubsd`/`vmulsd`/`vdivsd` (`VADDSD_XMMdq_XMMdq_MEMq` etc., XED extension `AVX`, four records per x86 profile). This needed no new `Lowered` representation, only generalizing the existing `Vex_binop_rrr`'s `src2` field from a bare `Reg.t` to a full `Rm.t` (renamed `Vex_binop_rr_rm` to say so honestly) and reusing `rm_codec` directly for it - VEX's ModR/M and SIB bytes are byte-for-byte identical to legacy SSE's, only the leading prefix bytes differ, so no new codec machinery was needed beyond a `vex_rm_ok` guard mirroring the register form's own xmm0-7 restriction: a memory operand's base/index (if present) must stay among the low 8 GPRs, since the two-byte VEX prefix has no REX.X/B-equivalent extension bit; confirmed against real GNU as that an r8-r15 base is silently re-encoded with the three-byte VEX prefix instead (`vaddsd (%r8), %xmm1, %xmm0` -> `c4 c1 73 58 00`), which this project's encoder correctly refuses rather than silently mis-encoding. Also added the family's bare-symbol source form (`vaddsd sym, %xmm1, %xmm0` on x86-32, `vaddsd sym(%rip), %xmm1, %xmm0` on x86-64), the same `mem_of_symbol` duality `movsd`/`movss`/`xorpd` already use. All new forms confirmed byte-for-byte against real GNU as (register-indirect, SIB-indexed, and symbolic addressing, on both `i686-linux-gnu-as` and `x86_64-linux-gnu-as` 2.44) before and after implementation. AVX now stands at promoted-support 8/676 (x86-32) and 8/702 (x86-64), double the register-register-only milestone's count. The second deferred follow-up - YMM (VEX.L), three-byte VEX, and EVEX - remains open, each its own distinct, larger future slice. GEN-05 has since fixed a diagnostic gap this VEX work exposed: `Isa_gen_oracle.observed_form_check`'s x86 leg assumed GAS's leading assembled byte was always the record's own plain opcode, true only for legacy `opcode_map=0`, no-mandatory-prefix forms; every VEX-encoded case's leading byte is instead the VEX prefix (`0xC5`/`0xC4`), so this reported a spurious `different_observed_form` mismatch for `vaddsd`/etc. even though the full byte sequence, and therefore `asm-isa-difficult-check`'s pass/fail verdict, were already correct. Fixed narrowly: for `space = "vex"` records the checker now detects the 2-byte-vs-3-byte VEX prefix from GAS's own leading byte and compares the opcode field at the position that actually follows it, leaving every legacy-space comparison byte-for-byte unchanged (a wider, pre-existing, VEX-unrelated version of this same gap - 162 of 328 x86 corpus cases already mismatch this check purely from legacy mandatory prefixes like `ADDSD`'s own `F2` - was found but deliberately left out of scope, a named follow-up rather than silently folded in). GEN-05 has since admitted the register<-memory form of `vaddsd`/`vsubsd`/`vmulsd`/`vdivsd` (`VADDSD_XMMdq_XMMdq_MEMq` etc., 4 records per x86 profile): no new `Lowered` representation, only generalizing the existing `Vex_binop_rrr`'s `src2` field from a bare `Reg.t` to a full `Rm.t` (renamed `Vex_binop_rr_rm` to say so honestly) and reusing `rm_codec` directly for it - VEX's ModR/M and SIB bytes are byte-for-byte identical to legacy SSE's, only the leading prefix bytes differ, so no new codec machinery was needed beyond a `vex_rm_ok` guard mirroring the register form's own xmm0-7 restriction: a memory operand's base/index (if present) must stay among the low 8 GPRs, since the two-byte VEX prefix has no REX.X/B-equivalent extension bit; confirmed against real GNU as that an r8-r15 base is silently re-encoded with the three-byte VEX prefix instead (`vaddsd (%r8), %xmm1, %xmm0` -> `c4 c1 73 58 00`), which this project's encoder correctly refuses rather than silently mis-encoding. Also added the family's bare-symbol source form (`vaddsd sym, %xmm1, %xmm0` on x86-32, `vaddsd sym(%rip), %xmm1, %xmm0` on x86-64), the same `mem_of_symbol` duality `movsd`/`movss`/`xorpd` already use. AVX then stood at promoted-support 8/676 (x86-32) and 8/702 (x86-64), double the register-register-only milestone's count. GEN-05 has since admitted `vaddsd`'s scalar-single (`VEX.LIG.F3.0F.WIG`) siblings `vaddss`/`vsubss`/`vmulss`/`vdivss` in both register-register and register<-memory forms (8 records per x86 profile, XED's own "d" (dword/32-bit) iform suffix replacing "q" (qword/64-bit) since single precision is 32 bits): a purely mechanical prefix-variant extension differing only in the VEX codec's `pp` field (2, not 3) - `vex_scalar_f2_rrr_alt` generalized into `vex_scalar_rrr_alt ~pp ~opcode_codec` (mirroring how `sse_binop_alt` is already generalized over `~mandatory`/`~opcode_codec`) plus a new `vex_scalar_f3_codec` opcode table, with `Lowered.Vex_binop_rr_rm`, `vex_rm_ok`, and both `vex_binop_rrr_form`/`vex_binop_rr_mem_form` normalizers reused completely unchanged. Confirmed against real GNU as before writing code: `vaddss %xmm2,%xmm1,%xmm0` -> `c5 f2 58 c2`, `vsubss %xmm7,%xmm3,%xmm5` -> `c5 e2 5c ef`, matching exactly once implemented. AVX now stands at promoted-support 24/676 (x86-32) and 24/702 (x86-64). Every scalar VEX arithmetic mnemonic sharing this three-real-register shape is now closed. GEN-05 has since admitted `vaddps`/`vsubps`/`vmulps`/`vdivps` (`pp = 0`, no mandatory prefix) and `vaddpd`/`vsubpd`/`vmulpd`/`vdivpd` (`pp = 1`, mandatory `66`), completing the VEX `pp` square (F2/F3/none/66) for opcodes `0x58`/`0x59`/`0x5C`/`0x5E` the way the legacy `ADDPS`/`ADDPD` slice already completed the same square for the non-VEX encoding: two new opcode tables (`vex_scalar_none_codec`/`vex_scalar_66_codec`) plug into the existing `vex_scalar_rrr_alt`/`Vex_binop_rr_rm`/`vex_rm_ok` machinery unchanged - pure mechanical extension, no new representation or shape. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44), identical on both targets: `vaddps %xmm2,%xmm1,%xmm0` -> `c5 f0 58 c2`, `vsubps %xmm7,%xmm3,%xmm5` -> `c5 e0 5c ef`, `vaddpd %xmm2,%xmm1,%xmm0` -> `c5 f1 58 c2`, and likewise for `mul`/`div` at `0x59`/`0x5E`, plus the register<-memory direction on both targets (e.g. `vaddps 16(%esp),%xmm1,%xmm0` -> `c5 f0 58 44 24 10`). Both register-register and register<-memory forms admitted together (16 records per x86 profile, 8 mnemonics x 2 directions). AVX (the XED family) now stands at promoted-support 32/676 (x86-32) and 32/702 (x86-64); x86 total promoted-support (all families combined) is now 190/7887 (x86-32) and 196/10571 (x86-64). GEN-05 has since admitted the VEX bitwise-logical family `vandps`/`vandnps`/`vorps`/`vxorps`/`vandpd`/`vandnpd`/`vorpd`/`vxorpd` (opcodes 0x54-0x57, `pp=0`/`pp=1`), the VEX counterpart of the already-admitted legacy `ANDPS`/`ANDNPS`/`ORPS`/`XORPS`/`ANDPD`/`ANDNPD`/`ORPD` family, and unlike ADD/SUB/MUL/DIV there is no scalar (F2/F3) sibling group for this family since AND/OR/XOR are inherently packed-only bitwise operations - reusing the existing `Vex_binop_rr_rm` shape and `vex_scalar_rrr_alt`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` machinery verbatim, with only two new codec-table rows (one per `pp` group) and the corresponding `Opcode.t` variants/admission wiring. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing code: `vandps %xmm2,%xmm1,%xmm0` -> `c5 f0 54 c2`, `vxorpd %xmm2,%xmm1,%xmm0` -> `c5 f1 57 c2`, and the register<-memory direction (e.g. `vandps 16(%esp),%xmm1,%xmm0` -> `c5 f0 54 44 24 10`) on both targets; also confirmed the same 2-byte-VEX xmm0-7 restriction applies to this family's register-register `src2` exactly as it does for the arithmetic group (an xmm8+ `src2` makes real GNU as silently emit the unbuilt 3-byte VEX prefix instead). Both directions admitted together (16 records per x86 profile, 8 mnemonics x 2 directions). AVX now stands at promoted-support 48/676 (x86-32) and 48/702 (x86-64); x86 total promoted-support (all families combined) is now 206/7887 (x86-32) and 212/10571 (x86-64). YMM (VEX.L), three-byte VEX, and EVEX remain the large, unscoped follow-ups. A fresh survey of the legacy (non-VEX) arc then found `MAXSS`/`MINSS`/`MAXSD`/`MINSD` (scalar min/max) and their packed siblings `MAXPS`/`MINPS`/`MAXPD`/`MINPD` had been overlooked across every earlier arithmetic-family survey pass: the same four-prefix-group shape (F3/F2/none/66) as `ADDSD`/`ADDSS`/`ADDPS`/`ADDPD`, just at opcode bytes 0x5F (max)/0x5D (min) rather than 0x58/0x59/0x5C/0x5E. GEN-05 now admits all 8, register-register and register<-memory (32 records per x86 profile total), reusing `xmm_binop_rr_form`/`xmm_binop_rm_form` and the existing `sse_binop_f2_codec`/`sse_binop_f3_codec`/`sse_binop_66_codec`/`sse_binop_none_codec` opcode tables verbatim - pure mechanical table extension, no new representation - confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) for every mnemonic and direction before writing code (`F3 0F 5F/5D`, `F2 0F 5F/5D`, `0F 5F/5D`, `66 0F 5F/5D`). SSE now stands at promoted-support 38/98 (x86-32) and 40/98 (x86-64); SSE2 at 52/263 (x86-32) and 56/268 (x86-64); x86 total promoted-support is now 222/7887 (x86-32) and 228/10571 (x86-64). GEN-05 has since admitted the VEX sibling of that same MAX/MIN family - `vmaxsd`/`vminsd` (VEX.LIG.F2.0F.WIG, pp=3), `vmaxss`/`vminss` (VEX.LIG.F3.0F.WIG, pp=2), `vmaxps`/`vminps` (VEX.128.0F.WIG, pp=0), `vmaxpd`/`vminpd` (VEX.128.66.0F.WIG, pp=1), all at opcode bytes 0x5F (max)/0x5D (min) - register-register and register<-memory together (32 records per x86 profile), reusing `Vex_binop_rr_rm`/`vex_scalar_rrr_alt` and the existing `vex_scalar_f2_codec`/`vex_scalar_f3_codec`/`vex_scalar_none_codec`/`vex_scalar_66_codec` opcode tables verbatim - pure mechanical table extension, no new representation or normalization/admission shape, mirroring exactly how the legacy MAX/MIN slice extended `ADDSD`/`ADDPS`'s own tables - confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) for every mnemonic and direction before writing code (`c5 f3 5f/5d` sd, `c5 f2 5f/5d` ss, `c5 f0 5f/5d` ps, `c5 f1 5f/5d` pd). AVX now stands at promoted-support 64/676 (x86-32) and 64/702 (x86-64); x86 total promoted-support is now 238/7887 (x86-32) and 244/10571 (x86-64). YMM (VEX.L), three-byte VEX, and EVEX remain the large, unscoped follow-ups; no further named small bounded VEX slice remains after this one. GEN-05 has since admitted the legacy `SQRTSS`/`SQRTSD`/`SQRTPS`/`SQRTPD` family (opcode 0x51, F3/F2/none/66 prefix groups) - the first genuinely unary-shaped member of this whole plain-binop arc: XED still marks the scalar forms' destination register `rw` rather than `w` (a scalar op preserves the destination's untouched upper bits, the same convention `ADDSD`'s own REG0 already has), so both `xmm_binop_rr_form`/`xmm_binop_rm_form` and the encoder's `Sse_binop_r_rm` shape apply completely unchanged despite the operation itself only reading one operand architecturally - register-register and register<-memory together (32 records per x86 profile), reusing the existing four prefix-group opcode tables (`sse_binop_f2_codec`/`sse_binop_f3_codec`/`sse_binop_66_codec`/`sse_binop_none_codec`) verbatim with one new row each, no new representation. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) for every mnemonic and direction before writing code (`F3 0F 51`, `F2 0F 51`, `0F 51`, `66 0F 51`). SSE now stands at promoted-support 42/98 (x86-32) and 44/98 (x86-64); SSE2 at 56/263 (x86-32) and 60/268 (x86-64); x86 total promoted-support is now 246/7887 (x86-32) and 252/10571 (x86-64). The VEX sibling `vsqrtss`/`vsqrtsd`/`vsqrtps`/`vsqrtpd` is a deliberately not-yet-verified follow-up: Intel's scalar VEX unary-ish square-root forms carry a real, required `VEX.vvvv` operand copying the source's upper bits rather than reducing to a true two-operand shape, and that semantic needs confirming against real GNU as before any code is written, not assumed from the legacy precedent. GEN-05 has since verified and closed that follow-up: decoding real GNU as's own assembled bytes by hand (`vsqrtsd %xmm3, %xmm2, %xmm1` -> `c5 eb 51 cb`, `vsqrtss %xmm3, %xmm2, %xmm1` -> `c5 ea 51 cb`) confirmed the predicted semantics exactly - `VEX.vvvv` is real and required for the scalar forms (`%xmm2` there, the merge-source, decodes out of the `vvvv` field precisely where `vmaxsd`'s own `src1` does), so `vsqrtsd`/`vsqrtss` reuse `Vex_binop_rr_rm`/`vex_scalar_rrr_alt` and the existing `vex_scalar_f2_codec`/`vex_scalar_f3_codec` opcode tables completely unchanged, needing only two new table rows (opcode `0x51`) - this project's encoder only assembles bytes, so it does not need to model *why* `vvvv` is real (a hardware upper-bits merge, not a second arithmetic input) to reuse the identical representation. The packed forms confirmed the opposite prediction just as cleanly: real GNU as rejects a third operand for `vsqrtps`/`vsqrtpd` outright ("number of operands mismatch"), and their assembled bytes (`vsqrtps %xmm2, %xmm1` -> `c5 f8 51 ca`, `vsqrtpd %xmm2, %xmm1` -> `c5 f9 51 ca`) decode `VEX.vvvv` as the literal `1111` ("unused") bit pattern - a genuinely new two-operand shape, `Lowered.Vex_unop_r_rm` (`dst := f(src)`, no `src1` field at all) plus a new `vex_unop_alt` codec function mirroring `vex_scalar_rrr_alt` with `vvvv` fixed at `0xF` instead of derived from an operand, and two new one-entry opcode tables (`vex_unop_none_codec`/`vex_unop_66_codec`, `pp = 0`/`pp = 1`) since opcode `0x51` doesn't collide with the existing four scalar-binop tables' entries. The two-byte-VEX xmm0-7 restriction on the r/m operand applies identically to this new shape (`vsqrtps %xmm8, %xmm1` is rejected the same way `vaddsd`'s own extended-register check already rejects one), confirmed before writing the guard. All four mnemonics admitted register-register and register<-memory, both x86-32/x86-64 (`VSQRTSD_XMMdq_XMMdq_XMMq`/`_MEMq`, `VSQRTSS_XMMdq_XMMdq_XMMd`/`_MEMd`, `VSQRTPS_XMMdq_XMMdq`/`_MEMdq`, `VSQRTPD_XMMdq_XMMdq`/`_MEMdq`), all cases passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`. AVX now stands at promoted-support 72/676 (x86-32) and 72/702 (x86-64); x86 total promoted-support is now 254/7887 (x86-32) and 260/10571 (x86-64). YMM (VEX.L), three-byte VEX, and EVEX remain the large, unscoped follow-ups. GEN-05 now also admits `vmovaps`/`vmovups`/`vmovapd`/`vmovupd` (opcodes 0x28/0x10, none/66 `pp`), the VEX sibling of the already-closed legacy `MOVAPS`/`MOVUPS`/`MOVAPD`/`MOVUPD` family, reusing `Lowered.Vex_unop_r_rm`/`vex_unop_alt` verbatim (the same genuinely-two-operand shape `vsqrtps`/`vsqrtpd` already established) with just two new opcode rows in each of the existing `pp=0`/`pp=1` codec tables - zero new representation. Confirmed by hand-decoding real GNU as bytes (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing code that register-register (`vmovaps %xmm2, %xmm1` -> `c5 f8 28 ca`) and register<-memory (`c5 f8 28 08`/`c5 f8 10 08`/`c5 f9 28 08`/`c5 f9 10 08`) both use this opcode; register-register is also reachable through a redundant `_29`/`_11`-suffixed iform (confirmed present in the XED resolved export, e.g. `VMOVAPS_XMMdq_XMMdq_29`) that real GNU as never selects for two register operands, matching the "low-numbered iform" precedent `Movapd`'s own legacy admission already established - left unadmitted here too, exactly like the legacy family. The real `MEMdq<-XMMdq` store direction (opcodes 0x29/0x11) is a genuinely distinct, separately admittable opcode rather than a redundancy (confirmed against the XED resolved export's own `VMOVAPS_MEMdq_XMMdq` etc. iforms) and is left as a named follow-up, matching the legacy family's own scope exactly rather than assuming VEX changes that answer. All 8 new records (4 mnemonics x 2 directions) admitted on both x86-32/x86-64, all cases passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`. AVX now stands at promoted-support 80/676 (x86-32) and 80/702 (x86-64); SSE/SSE2 are unchanged by this slice (the legacy family was already closed). GEN-05 now also admits legacy `ucomiss` (opcode 0x2E, no mandatory prefix), `Comiss`'s own exact sibling overlooked when the comparison family was first admitted alongside `Ucomisd` - a plain addition to `sse_binop_none_codec` and the `xmm_binop_rr_form`/`xmm_binop_rm_form` lookup lists, zero new representation, confirmed against real GNU as (`0f 2e ca` register-register, `0f 2e 08` register<-memory) before writing code - and the VEX comparison family `vcomisd`/`vucomisd`/`vcomiss`/`vucomiss` (opcodes 0x2F/0x2E, 66/none `pp`), the VEX sibling of the legacy `COMISD`/`UCOMISD`/`COMISS`/`UCOMISS` family, reusing `Lowered.Vex_unop_r_rm`/`vex_unop_alt` verbatim (two new opcode rows in each of the existing `pp=0`/`pp=1` codec tables): confirmed against real GNU as that both operands are read-only here (XED's own REG0 is `rw = "r"`, not "w" - the real result goes to EFLAGS, not a register) but `vex_unop_rr_form` already reads `role_of_rw` generically off each operand's own `rw` fact rather than assuming the destination is writable, so this is a clean reuse rather than a new shape; confirmed bytes (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44): `c5 f9 2f ca`/`c5 f9 2e ca` (vcomisd/vucomisd register-register), `c5 f8 2f ca`/`c5 f8 2e ca` (vcomiss/vucomiss register-register), and the matching four register<-memory forms. All 12 new records (2 legacy directions plus 4 VEX mnemonics x 2 directions) admitted on both x86-32/x86-64, all cases passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`. SSE now stands at promoted-support 44/98 (x86-32) and 46/98 (x86-64); AVX now stands at promoted-support 88/676 (x86-32) and 88/702 (x86-64); SSE2 is unchanged by this slice. GEN-05 now also admits `cvtps2pd`/`cvtpd2ps` (opcode 0x5A, the same none/66 mandatory-prefix pair that already distinguishes `CVTSD2SS`/`CVTSS2SD`'s own F2/F3 rungs), reusing `xmm_binop_rr_form`/`xmm_binop_rm_form` verbatim in both directions - confirmed against real GNU as that, unlike the VEX sibling below, both register-register and register<-memory are unambiguous here, since this project builds no competing wider-operand form for either mnemonic at the legacy encoding - and the VEX sibling `vcvtps2pd`/`vcvtpd2ps`, reusing `Lowered.Vex_unop_r_rm`/`vex_unop_alt`'s existing none/66 tables unchanged: `vcvtps2pd` admits both directions (`c5 f8 5a ca` register-register, `c5 f8 5a 08` register<-memory, confirmed unambiguous since XMM-vs-YMM destination class alone picks the source width), while `vcvtpd2ps` admits only its register-register iform (`c5 f9 5a ca`) - its two vex-space memory iforms (`VCVTPD2PS_XMMdq_MEMdq`'s VEX.128 m128 source and `VCVTPD2PS_XMMdq_MEMqq`'s VEX.256 m256 source) render as the identical bare GAS spelling `vcvtpd2ps mem, %xmmN`, which real GNU as rejects outright ("operand size mismatch") absent the `x`/`y`-suffix disambiguation (`vcvtpd2psx`/`vcvtpd2psy`, confirmed accepted separately) this project's parser does not implement - left as a named follow-up alongside every other 3-byte-VEX/YMM/EVEX scope. All 7 new records (4 legacy, 3 VEX) admitted on both x86-32/x86-64, all cases passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`. SSE2 now stands at promoted-support 60/263 (x86-32) and 64/268 (x86-64); AVX now stands at promoted-support 91/676 (x86-32) and 91/702 (x86-64); SSE is unchanged by this slice. GEN-05 now also admits the packed-interleave family `unpcklps`/`unpckhps`/`unpcklpd`/`unpckhpd` (opcodes 0x14/0x15, none/66 mandatory prefix, `ANDPS`'s own mandatory-prefix-free/66 group at a different opcode byte) and its VEX sibling `vunpcklps`/`vunpckhps`/`vunpcklpd`/`vunpckhpd`: the first genuine two-source-operand packed binop admitted since `ANDPS`/`VANDPS` themselves, so both directions reuse `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Lowered.Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged - only two new opcode-table rows in each of the four existing legacy prefix tables and four existing VEX `pp`-variant tables. XED's own iform naming is asymmetric here in a way worth noting without it affecting anything: `UNPCKLPS`'s register-register iform is `XMMps_XMMq` (a 64-bit source) while `UNPCKHPS`'s is `XMMps_XMMdq` (128-bit) - an upstream naming quirk with no encoding consequence, confirmed identical `0F 14 /r`/`0F 15 /r` bytes either way. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `unpcklps %xmm2, %xmm1` -> `0f 14 ca`, `unpckhpd %xmm2, %xmm1` -> `66 0f 15 ca`, `vunpcklps %xmm3, %xmm2, %xmm1` -> `c5 e8 14 cb`, `vunpckhpd %xmm3, %xmm2, %xmm1` -> `c5 e9 15 cb`, and the matching register<-memory forms and x86-64 REX/xmm8-15 rejections (confirmed the existing `vex_rm_ok` guard already covers this family with no changes: `vunpcklps (%r8), %xmm2, %xmm1` needs the still-unbuilt three-byte VEX prefix, exactly like every prior VEX family). 32 new records (8 legacy: 4 mnemonics x 2 directions, 8 VEX: 4 mnemonics x 2 directions, x2 for both x86 profiles) admitted, all passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`/`check`. SSE now stands at promoted-support 48/98 (x86-32) and 50/98 (x86-64); SSE2 now stands at 64/263 (x86-32) and 68/268 (x86-64); AVX now stands at promoted-support 99/676 (x86-32) and 99/702 (x86-64). GEN-05 now also admits `punpcklqdq`/`punpckhqdq` (opcode 0x6C/0x6D, 66-mandatory-prefix only - the first packed-interleave sibling with no non-66 counterpart, since this is integer SIMD rather than a float one, matching `PXOR`'s own precedent) and its VEX sibling `vpunpcklqdq`/`vpunpckhqdq`: the exact same shape as `UNPCKLPS`/`VUNPCKLPS`, reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Lowered.Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged - only two new opcode-table rows in the existing 66-mandatory-prefix legacy table and the existing VEX `pp=1` table. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `punpcklqdq %xmm2, %xmm1` -> `66 0f 6c ca`, `punpckhqdq %xmm2, %xmm1` -> `66 0f 6d ca`, `vpunpcklqdq %xmm3, %xmm2, %xmm1` -> `c5 e9 6c cb`, `vpunpckhqdq %xmm3, %xmm2, %xmm1` -> `c5 e9 6d cb`, identical bytes on both x86-32 and x86-64. 8 new records (4 legacy: 2 mnemonics x 2 directions, 4 VEX: 2 mnemonics x 2 directions) admitted, all passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`/`check`. SSE unchanged (this family is SSE2-only); SSE2 now stands at promoted-support 68/263 (x86-32) and 72/268 (x86-64); AVX now stands at promoted-support 103/676 (x86-32) and 103/702 (x86-64). GEN-05 now also admits `shufps`/`shufpd` (opcode 0xC6, none/66 mandatory prefix) and their VEX siblings `vshufps`/`vshufpd`: the first XMM-immediate-carrying shape in either arc, since every prior admitted family had at most two real register/memory operands. The legacy pair needed a new `Lowered.Sse_binop_imm_r_rm` (`Sse_binop_r_rm`'s own `reg`/`rm` pair plus a trailing `imm : int64`, encoded via new `sse_shuf_alt`/`sse_shuf_none_alt` functions built directly from `shld_imm_form`'s existing two-byte-opcode-plus-trailing-imm8 precedent spliced with `sse_binop_alt`'s mandatory-prefix layout); the VEX pair needed the equivalent `Lowered.Vex_binop_imm_rr_rm` (`Vex_binop_rr_rm`'s own three-operand shape plus the same trailing `imm`, via a new `vex_scalar_imm_rrr_alt`). A real decode-ambiguity bug surfaced and was fixed while building the legacy pair: `sse_shuf_none_alt` (mandatory-prefix-free, reusing `prefixes_codec` directly per `sse_binop_none_alt`'s own precedent) can spuriously consume a genuine leading mandatory `0x66` byte through its own generic `opsz_codec` field, so `sse-shufpd` (the true 66-prefixed alt) must be given a lower priority number than `sse-shufps` and tried first during decode - exactly the reason `sse_binop_alt`'s own 66 group (priority 26) already precedes `sse_binop_none_alt` (27); this project's own VEX alts are immune to the same ambiguity since their `pp` field is checked explicitly against the VEX prefix's own byte2, not inferred from an optional leading byte. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code, including the extended-register (`%xmm9`+) and 2-byte-VEX `src2` xmm0-7 rejection cases: `shufps $0x1b,%xmm2,%xmm1` -> `0f c6 ca 1b`, `shufpd $0x1,%xmm2,%xmm1` -> `66 0f c6 ca 01`, `vshufps $0x1b,%xmm3,%xmm2,%xmm1` -> `c5 e8 c6 cb 1b`, `vshufpd $0x1,%xmm3,%xmm2,%xmm1` -> `c5 e9 c6 cb 01`, and the matching register<-memory forms on both x86-32/x86-64. 16 new records (8 legacy: 2 mnemonics x 2 directions x 2 profiles, 8 VEX: 2 mnemonics x 2 directions x 2 profiles) admitted, all passing byte-for-byte against real GNU as via `asm-isa-difficult-regen`/`check`. SSE now stands at promoted-support 50/98 (x86-32) and 52/98 (x86-64); SSE2 now stands at 70/263 (x86-32) and 74/268 (x86-64); AVX now stands at promoted-support 107/676 (x86-32) and 107/702 (x86-64). GEN-05 now also admits `CMPSS`/`CMPSD`/`CMPPS`/`CMPPD` (opcode 0xC2, the same four-mandatory-prefix-group shape as `ADDSD`/`ADDSS`/`ADDPS`/`ADDPD` at a different opcode byte, reusing `Sse_binop_imm_r_rm`/`xmm_binop_imm_rr_form`/`xmm_binop_imm_rm_form` unchanged - pure mechanical extension into the existing four legacy prefix-group opcode tables, only the canonical `cmp{ss,sd,ps,pd} $imm, ...` spelling, not the `cmpeq`/`cmplt`/etc. mnemonic-suffix pseudo-aliases real GNU as also accepts) and their VEX siblings `vcmpss`/`vcmpsd`/`vcmpps`/`vcmppd` (generalizing the prior VSHUFPS-only `vex_scalar_imm_rrr_alt`/`sse_shuf_alt`/`sse_shuf_none_alt` into table-driven `vex_binop_imm_rrr_alt`/`sse_binop_imm_alt`/`sse_binop_imm_none_alt` the same way the scalar-arithmetic/bitwise-logical families were generalized earlier, since `CMPSD`/`CMPSS` each need their own one-entry mandatory-F2/F3 group at opcode 0xC2 while `CMPPS`/`CMPPD` join `SHUFPS`/`SHUFPD`'s own none/66 groups), confirmed byte-for-byte against real GNU as on both x86-32/x86-64 (register-register and register<-memory) before writing any code - e.g. `cmpss $0x0,%xmm2,%xmm1` -> `f3 0f c2 ca 00`, `vcmpss $0x0,%xmm2,%xmm1,%xmm0` -> `c5 f2 c2 c2 00`. Also confirmed the legacy `CMPSD_XMM_XMMsd_XMMsd_IMMb` iform is disjoint from the unrelated bare-`CMPSD` string-instruction iform sharing the same mnemonic spelling, avoiding a dispatch collision. 32 new records (16 legacy: 4 mnemonics x 2 directions x 2 profiles, 16 VEX: 4 mnemonics x 2 directions x 2 profiles). SSE now stands at promoted-support 54/98 (x86-32) and 56/98 (x86-64); SSE2 now stands at 74/263 (x86-32) and 78/268 (x86-64); AVX now stands at promoted-support 115/676 (x86-32) and 115/702 (x86-64). GEN-05 now also admits `pshufd`/`pshuflw`/`pshufhw` (packed shuffle with an imm8 control byte, opcode 0x70, 66/F2/F3 mandatory-prefix groups - no mandatory-prefix-free sibling, since that slot belongs to the unrelated MMX instruction `pshufw`, a different register class out of scope): genuinely unary (XED marks the destination `w`, not `rw`, unlike `SHUFPS`'s destructive read-write `reg`), but the ModR/M-reg/ModR/M-rm/trailing-imm8 byte layout is identical, so both reuse `Lowered.Sse_binop_imm_r_rm`/`xmm_binop_imm_rr_form`/`xmm_binop_imm_rm_form` completely unchanged - `role_of_rw` already derives operand roles from XED's own `rw` fact rather than assuming read-write, so this closes as pure admission wiring plus three new opcode-table rows in the existing legacy 66/F2/F3 prefix tables, no encoder change. Their VEX siblings `vpshufd`/`vpshuflw`/`vpshufhw` are genuinely two-operand-plus-immediate (no real `vvvv` operand at all, confirmed against real GNU as, which rejects a third register operand outright the same way it does for `vsqrtps`'s own non-immediate packed unary forms) - a new `Lowered.Vex_unop_imm_r_rm`/`vex_unop_imm_alt` shape, mirroring `Vex_unop_r_rm`'s own no-`vvvv` convention (literal `1111` bit pattern) plus `Vex_binop_imm_rr_rm`'s trailing imm8, and correspondingly new `vex_unop_imm_rr_form`/`vex_unop_imm_rm_form` normalizers mirroring `vex_unop_rr_form`/`vex_unop_rr_mem_form`. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code, identical on both targets: `pshufd $0x1b,%xmm2,%xmm1` -> `66 0f 70 ca 1b`, `pshuflw`/`pshufhw` at the same opcode with `F2`/`F3`, `pshufd $0x1b,16(%esp|%rsp),%xmm1` -> `66 0f 70 4c 24 10 1b`, `vpshufd $0x1b,%xmm2,%xmm1` -> `c5 f9 70 ca 1b`, `vpshuflw`/`vpshufhw` with `pp=3`/`pp=2` respectively, and the register<-memory direction for all six (e.g. `vpshufd $0x1b,16(%esp|%rsp),%xmm0` -> `c5 f9 70 44 24 10 1b`); also confirmed this project's own encoder correctly rejects `vpshufd $0x1b,%xmm9,%xmm1` (xmm8-15 in the two-byte-VEX r/m slot), matching the existing `vex_rm_ok` precedent - real GNU as instead silently upgrades that spelling to a three-byte VEX prefix, which this project does not yet build. 24 new records total (12 legacy: 3 mnemonics x 2 directions x 2 profiles, 12 VEX: 3 mnemonics x 2 directions x 2 profiles). SSE2 now stands at promoted-support 80/263 (x86-32) and 84/268 (x86-64); AVX now stands at promoted-support 121/676 (x86-32) and 121/702 (x86-64); SSE is unchanged (this family has no scalar/SSE-only member). GEN-05 now also admits legacy `movd`/`movq` (`66 0F 6E /r` load, `66 0F 7E /r` store, GEN-05's first mixed-GPR/XMM data-move family since the earlier `cvtsi2sd`/`cvttsd2si` conversion arc): one shared `Opcode.Movd` across both mnemonics and both directions, exactly like `Cvtsi2sd`/`cvtsi2sdq` (`width` selects REX.W). The load direction reuses `Lowered.Cvtsi2f_r_rm` unchanged (byte-for-byte the same ModR/M-reg=xmm/ModR/M-rm=gpr-or-mem shape, only the opcode byte and mandatory prefix differ from `cvtsi2sd`'s `0F 2A`), generalizing `sse_cvtsi2f_alt` with a new `~opcode16` parameter; the store direction needed one genuinely new shape, `Lowered.Movd_rm_r` (`Cvtsi2f_r_rm`'s own field layout with `reg`/`rm`'s AT&T print order reversed, since `reg` - xmm - is the source here rather than the destination) and its own `sse_movd_store_alt` codec function. Confirmed byte-for-byte against real GNU as before writing any code, both directions, both widths, register and memory operands, both x86-32/x86-64 (`movd %eax,%xmm0` -> `66 0f 6e c0`; `movd %xmm0,%eax` -> `66 0f 7e c0`; memory forms byte-identical on both profiles; `movq %rax,%xmm0`/`movq %xmm0,%rax` -> `66 48 0f 6e c0`/`66 48 0f 7e c0` on x86-64 only, REX.W; `movq %eax,%xmm0` rejected outright in 32-bit mode with "operand type mismatch"). Unlike `cvtsi2sd`/`cvtsi2sdq`, MOVQ's 64-bit-only applicability needed no derived requirement at all: its own `provenance.mode_restriction`/`applicability` (`equals: "mode64"`) is already a native per-record XED fact, confirmed by inspecting the checked-in export directly. MOVQ's memory-operand forms are deliberately NOT admitted: confirmed against real GNU as that `movq (%rax),%xmm0` assembles to `f3 0f 7e 00` and `movq %xmm0,(%rax)` to `66 0f d6 00` - GAS always prefers the unrelated scalar-xmm `MOVQ xmm1, xmm2/m64` instruction (a genuinely different opcode sharing the "movq" mnemonic) whenever the other operand is memory rather than a specific-width GPR register, so the GPR64<->mem form of *this* instruction is unreachable via that spelling, mirroring `Vcvtpd2ps`'s own memory-operand-ambiguity precedent exactly. At the frontend, `movd` dispatches unconditionally (no `mov`-suffix collision - `d` is not a stripped width-suffix character), while `movq` - genuinely ambiguous with the plain 64-bit GPR `mov` - only routes here when an xmm operand (`width = 128`) is actually present in the operand list, falling through unchanged to the generic `mov` handler otherwise; confirmed this correctly leaves `movq %rax,%rbx` alone. 10 new records total (4 legacy MOVD forms x 2 profiles = 8, plus 2 legacy MOVQ register-register forms x86-64-only = 2). The VEX siblings `vmovd`/`vmovq` are deliberately deferred: `vmovd` (VEX.W0, GPR32<->xmm) is buildable with the existing 2-byte VEX infrastructure but needs a genuinely new VEX "store direction" shape (this project's VEX arc has so far only built the load-direction `Vex_unop_r_rm`/`Vex_binop_rr_rm` shapes) that was out of scope for this slice, while `vmovq` (VEX.W1, GPR64<->xmm) cannot be built with 2-byte VEX at all - the 2-byte VEX prefix's second byte has no W bit (only R/vvvv/L/pp, confirmed against the Intel encoding and this project's own `vex_scalar_rrr_alt`/`vex_unop_alt` byte layout), so VEX.W1 forms structurally require the 3-byte VEX prefix this project has not built yet. SSE2 now stands at promoted-support 84/263 (x86-32) and 90/268 (x86-64); SSE and AVX are unchanged at 54/98 (x86-32)/56/98 (x86-64) and 121/676 (x86-32)/121/702 (x86-64) respectively, since this slice's ten new records are all SSE2. GEN-05 has since admitted the VEX sibling `vmovd` (GPR32<->xmm only, VEX.128.66.0F.W0, opcode 0x6E load-direction / 0x7E store-direction), the deferred follow-up named in the legacy `movd`/`movq` milestone above: a genuinely new cross-register-class VEX shape (`Vex_movd_r_rm` for the load direction, `Vex_movd_rm_r` for the store direction, mirroring the legacy `Cvtsi2f_r_rm`/`Movd_rm_r` split), since every VEX shape built before this session (`Vex_binop_rr_rm`, `Vex_unop_r_rm`, and their immediate-carrying siblings) was XMM-to-XMM only. `vmovd`'s own normalization side needed zero new code: XED's `VMOVD_XMMdq_GPR32d`/`VMOVD_XMMdq_MEMd`/`VMOVD_GPR32d_XMMd`/`VMOVD_MEMd_XMMd` records have the exact same REG0/REG1/MEM0 operand-name shape as their legacy counterparts, so the pre-existing `movd_load_rr_form`/`movd_load_rm_form`/`movd_store_rr_form`/`movd_store_mr_form` normalizers were reused verbatim (just called with `~mnemonic:"vmovd"`), with `requirement_of` deriving `x86:avx` automatically from these records' own `extension: "AVX"` fact. Confirmed against real GNU as before writing any encoder code: `vmovd %eax,%xmm0` -> `c5 f9 6e c0`, `vmovd %xmm0,%eax` -> `c5 f9 7e c0`, both directions also confirmed for register<->memory. One correction to the legacy milestone's own assumption: real GNU as does NOT reject `vmovd %rax,%xmm0` the way it has no bare-`vmovq` mnemonic error - it silently reassembles that spelling as `vmovq` instead (`c4 e1 f9 6e c0`, three-byte VEX, VEX.W1), the same width-driven mnemonic substitution the legacy `movd`/`movq` dispatch already performs at the text-parsing level; this project does not perform that substitution (no three-byte-VEX/`vmovq` support exists), so a 64-bit GPR operand here is correctly rejected by the ordinary `width_ok` register-width mismatch check as a deliberate, named scope boundary, not a false rejection of a real GNU-as-accepted form under the `vmovd` spelling itself. `vmovq` (VEX.W1, GPR64<->xmm) remains out of scope, confirmed still structurally blocked on the three-byte VEX prefix this project has not built (the two-byte VEX prefix's second byte has no W bit at all). 8 new AVX records total (4 iforms x 2 profiles). AVX now stands at promoted-support 125/676 (x86-32) and 125/702 (x86-64); SSE and SSE2 are unchanged at this slice's own scope (SSE2 84/263 (x86-32)/90/268 (x86-64) as before), since all four new records are AVX-extension. GEN-05 now also admits the packed integer add/subtract family `paddb`/`paddw`/`paddd`/`paddq`/`psubb`/`psubw`/`psubd`/`psubq` (opcodes 0xFC/0xFD/0xFE/0xD4 add, 0xF8/0xF9/0xFA/0xFB subtract, 66-mandatory-prefix only - no non-66 sibling, since these are integer SIMD ops rather than float ones, the same shape `PUNPCKLQDQ`/`PUNPCKHQDQ` already established) and their VEX siblings `vpaddb`/`vpaddw`/`vpaddd`/`vpaddq`/`vpsubb`/`vpsubw`/`vpsubd`/`vpsubq`, reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged in both arcs - pure mechanical opcode-table extension into the existing 66-mandatory-prefix legacy table and the existing VEX `pp=1` (66) table, no new representation needed anywhere. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `paddb %xmm2,%xmm1` -> `66 0f fc ca` (both x86-32/x86-64, byte-identical), `vpaddb %xmm3,%xmm2,%xmm1` -> `c5 e9 fc cb`, and the register<-memory direction for all sixteen mnemonics. 32 new records per x86 profile (16 legacy + 16 VEX, 8 mnemonics x 2 directions each). SSE2 now stands at promoted-support 100/263 (x86-32) and 106/268 (x86-64); AVX now stands at promoted-support 141/676 (x86-32) and 141/702 (x86-64). GEN-05 now also admits the packed compare-equal/greater-than family `pcmpeqb`/`pcmpeqw`/`pcmpeqd`/`pcmpgtb`/`pcmpgtw`/`pcmpgtd` (opcodes 0x74/0x75/0x76 equal, 0x64/0x65/0x66 greater-than, 66-mandatory-prefix only, `PADDB`'s own group at different opcode bytes) and their VEX siblings `vpcmpeqb`/`vpcmpeqw`/`vpcmpeqd`/`vpcmpgtb`/`vpcmpgtw`/`vpcmpgtd`, reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged in both arcs - pure mechanical opcode-table extension into the existing 66-mandatory-prefix legacy table and the existing VEX `pp=1` (66) table, no new representation needed anywhere, the identical pattern the immediately preceding `PADDB`/`VPADDB` slice established. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `pcmpeqb %xmm2,%xmm1` -> `66 0f 74 ca` (both x86-32/x86-64, byte-identical), `vpcmpeqb %xmm3,%xmm2,%xmm1` -> `c5 e9 74 cb`, and the register<-memory direction for all twelve mnemonics. 24 new records per x86 profile (12 legacy + 12 VEX, 6 mnemonics x 2 directions each). SSE2 now stands at promoted-support 112/263 (x86-32) and 118/268 (x86-64); AVX now stands at promoted-support 153/676 (x86-32) and 153/702 (x86-64). GEN-05 now also admits the pack-with-saturation family `packsswb`/`packssdw`/`packuswb` (opcodes 0x63/0x6B/0x67, 66-mandatory-prefix only - no non-66 sibling, `PCMPEQB`'s own group at different opcode bytes; only the plain-XMM iforms, since the same mnemonics' MMX (`MMXq`) iforms are a different, unrelated legacy register class this project doesn't admit anywhere) and their VEX siblings `vpacksswb`/`vpackssdw`/`vpackuswb` (only the plain-XMM iforms; the YMM/AVX512-masked iforms XED also exports for these mnemonics are out of scope), reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged in both arcs - pure mechanical opcode-table extension into the existing 66-mandatory-prefix legacy table and the existing VEX `pp=1` (66) table, no new representation needed anywhere, the identical pattern the `PADDB`/`PCMPEQB` slices established. Also confirmed the encoder has no `0F38`/`0F3A` three-byte-opcode infrastructure at all, ruling `PACKUSDW` (a genuinely different three-byte-opcode SSE4.1 instruction sharing no opcode byte with this family) out of this slice's scope, a separate not-yet-scoped follow-up rather than a gap in this family's own closure. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `packsswb %xmm2,%xmm1` -> `66 0f 63 ca` (both x86-32/x86-64, byte-identical), `vpacksswb %xmm3,%xmm2,%xmm1` -> `c5 e9 63 cb`, and the register<-memory direction for all six mnemonics. 12 new records per x86 profile (6 legacy + 6 VEX, 3 mnemonics x 2 directions each). SSE2 now stands at promoted-support 118/263 (x86-32) and 124/268 (x86-64); AVX now stands at promoted-support 159/676 (x86-32) and 159/702 (x86-64). GEN-05 now also admits the integer bitwise family `pand`/`pandn`/`por` (opcodes 0xDB/0xDF/0xEB, 66-mandatory-prefix only - no non-66 sibling, since these operate on the integer register interpretation with no packed-single counterpart, `pxor`'s own group at different opcode bytes) and the original SSE2-era packed integer min/max family `pminub`/`pmaxub`/`pminsw`/`pmaxsw` (opcodes 0xDA/0xDE/0xEA/0xEE, 66-mandatory-prefix only, `paddb`'s own group at yet more opcode bytes; only the plain-XMM iforms, since the same mnemonics' MMX (`MMXq`) iforms are a different, unrelated legacy register class this project doesn't admit anywhere), plus their VEX siblings `vpand`/`vpandn`/`vpor`/`vpminub`/`vpmaxub`/`vpminsw`/`vpmaxsw` (only the plain-XMM iforms; the YMM/AVX512-masked iforms XED also exports for these mnemonics are out of scope), reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged in both arcs - pure mechanical opcode-table extension into the existing 66-mandatory-prefix legacy table and the existing VEX `pp=1` (66) table, no new representation needed anywhere. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `pand %xmm2,%xmm1` -> `66 0f db ca`, `vpand %xmm3,%xmm2,%xmm1` -> `c5 e9 db cb`, `pminub %xmm2,%xmm1` -> `66 0f da ca`, `vpminub %xmm3,%xmm2,%xmm1` -> `c5 e9 da cb`, and the register<-memory direction for all fourteen mnemonics. 28 new records per x86 profile (14 legacy + 14 VEX, 7 mnemonics x 2 directions each). SSE2 now stands at promoted-support 132/263 (x86-32) and 138/268 (x86-64); AVX now stands at promoted-support 173/676 (x86-32) and 173/702 (x86-64). GEN-05 now also admits the packed-integer multiply/average/sum-of-absolute-differences family `pmullw`/`pmulhw`/`pmulhuw`/`pavgb`/`pavgw`/`psadbw` (opcodes 0xD5/0xE5/0xE4/0xE0/0xE3/0xF6, 66-mandatory-prefix only, `pand`'s own group at yet more opcode bytes; only the plain-XMM iforms, since the same mnemonics' MMX (`MMXq`) iforms are the same unrelated legacy register class already excluded elsewhere), plus their VEX siblings `vpmullw`/`vpmulhw`/`vpmulhuw`/`vpavgb`/`vpavgw`/`vpsadbw` (only the plain-XMM iforms; YMM/AVX512-masked iforms out of scope), reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged in both arcs - pure mechanical opcode-table extension into the existing 66-mandatory-prefix legacy table and the existing VEX `pp=1` (66) table. This slice also closed a real gap left by the immediately preceding `pand`/`pminub` slice: those mnemonics normalized successfully and passed every round-trip check, but never actually reached `promoted-support` status, because `Isa_family_admission.promoted_case`'s own hardcoded allow-list - a second, independent gate this project's admission accounting checks beyond successful normalization - had not been extended alongside them; this slice adds both the `pmullw`-family entries and confirms the fix by watching `SSE2`/`AVX`'s `normalized-only` counts fall back to their pre-`pand` baseline while `promoted-support` absorbs the difference, a concrete, measured instance of the "keep `isa_family_admission.ml`'s promoted-case allow-list in step" discipline this file has named as a recurring risk. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `pmullw %xmm2,%xmm1` -> `66 0f d5 ca`, `vpmullw %xmm3,%xmm2,%xmm1` -> `c5 e9 d5 cb`, `psadbw %xmm2,%xmm1` -> `66 0f f6 ca`, and the register<-memory direction for all twelve mnemonics. 24 new records per x86 profile (12 legacy + 12 VEX, 6 mnemonics x 2 directions each). SSE2 now stands at promoted-support 144/263 (x86-32) and 150/268 (x86-64); AVX now stands at promoted-support 185/676 (x86-32) and 185/702 (x86-64). GEN-05 now also admits `movdqa`/`movdqu` (integer/general XMM register move, `66 0F 6F`/`7F` and `F3 0F 6F`/`7F`, `movaps`'s integer-classified sibling) and their VEX siblings `vmovdqa`/`vmovdqu`, and this slice is the first to genuinely diverge from `movaps`'s own scope: `movdqa`/`movdqu`'s load (`0x6F`) and store (`0x7F`) opcodes are real, distinct bytes rather than one opcode shared by both directions, so - confirmed against the checked-in XED export directly, not assumed - the legacy pair reuses `movsd`/`movss`'s own two-direction `Lowered.Sse_mov_r_rm`/`Sse_mov_rm_r` shape via `sse_mov_r_rm_alt`/`sse_mov_rm_r_alt` unchanged (just new `mandatory`/`opcode16` arguments), extended with a new register-register lowering arm mapped to the load-direction opcode (real GNU as's own "low-numbered iform" choice, confirmed against the XED capture's own redundant `MOVDQA_XMMdq_XMMdq_0F7F` register-register-via-store-opcode iform, which is deliberately left unadmitted) - `xmm_binop_rr_form` normalizes that register-register iform verbatim, since a plain move's REG0 `rw="w"` is handled by the same generic `role_of_rw` dispatch a real binop's `rw="rw"` already uses. The VEX siblings stay load-direction-only (register-register plus register<-memory), matching `vmovaps`'s own precedent, and needed the first mandatory-`F3` `Lowered.Vex_unop_r_rm` codec table (`vmovdqu`; every prior `Vex_unop_r_rm` member used `pp=0`/`pp=1` only) alongside a plain new opcode-`0x6F` row in the existing `pp=1` table for `vmovdqa`. Confirmed against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code: `movdqa %xmm2,%xmm1` -> `66 0f 6f ca`, `movdqa (%eax),%xmm1` -> `66 0f 6f 08`, `movdqa %xmm1,(%eax)` -> `66 0f 7f 08`, `vmovdqa %xmm2,%xmm1` -> `c5 f9 6f ca`, `vmovdqu %xmm2,%xmm1` -> `c5 fa 6f ca`. This slice also caught and closed a second, distinct admission gap beyond the already-named `promoted_case` allow-list one: `Isa_gen_difficult`'s own hand-maintained case-entry lists (`x86_sse_binop_rr_entries`/`x86_sse_mov_entries`/`x86_vex_unop_rr_entries`/`x86_vex_unop_rr_mem_entries`, each a literal per-mnemonic lookup-key list, not something derived automatically from `promoted_case`) are a *third*, independent gate a mnemonic must be added to before it gets real GNU-as-differential case coverage in `asm/fixtures/isa-difficult/cases.jsonl` - confirmed by direct inspection that the immediately preceding `pmullw`/`pmulhw`/`pmulhuw`/`pavgb`/`pavgw`/`psadbw` slice (and its VEX siblings) normalize, promote, and pass every accounting check, yet have zero entries in any of these four lists and therefore zero actual cases in `cases.jsonl`, on either the legacy or VEX side; not fixed here (out of scope for this slice), flagged as a named follow-up with its own task. 20 new records per x86 profile (12 legacy + 8 VEX; VEX is load-direction-only, so no separate store-direction VEX record exists). SSE2 now stands at promoted-support 150/263 (x86-32) and 156/268 (x86-64); AVX now stands at promoted-support 189/676 (x86-32) and 189/702 (x86-64); SSE is unchanged (this family is entirely SSE2). Correction to that flagged follow-up (commits `46dd9ec`/`510c15d`, the `PMULLW`/`PMULHW`/`PMULHUW`/`PAVGB`/`PAVGW`/`PSADBW` legacy+VEX slice): those twelve mnemonics had genuinely normalized, passed `promoted_case`'s allow-list, and were manually confirmed byte-for-byte against real GNU as before that commit landed, but `Isa_gen_difficult`'s own case-entry lists (`x86_sse_binop_rr_entries`, `x86_sse_binop_rm_entries`, `x86_vex_binop_rrr_entries`, `x86_vex_binop_rr_mem_entries`) were never extended for them, so `asm/fixtures/isa-difficult/cases.jsonl` carried zero reusable, re-checkable differential cases for this family - the byte-for-byte claim was true as a one-time manual check but not retained as corpus evidence. Fixed by adding all 24 missing lookup keys (6 legacy register-register, 6 legacy register<-memory, 6 VEX register-register, 6 VEX register<-memory) to those four lists; `asm-isa-difficult-regen` now produces 48 new cases, all verdict `pass` (legacy forms show the same pre-existing opcode-byte "different_observed_form" diagnostic quirk every other mandatory-prefixed legacy family already has, not a regression). A full sweep cross-referencing every literal lookup-key string in `Isa_family_admission`'s `promoted_case` allow-list against `isa_gen_difficult.ml` found no other family with this gap after this fix - every promoted-case lookup key now has a matching case-entry registration somewhere in that file (a presence check, not a proof that each registration is wired into `Isa_gen_difficult.all` correctly, but a reasonable bound given the pattern is otherwise uniform across every family in this file). No promoted-support/normalized/blocked fractions changed by this fix, since it only affects case-corpus registration, not normalization or admission accounting; `tools-test`, `tools-integration`, `tools-boundary`, `asm-isa-difficult-regen`+`check` (1724 PASS, 0 FAIL), full `asm-test`, and `asm-fmt-check` all pass. GEN-05 now also admits `PINSRW`/`PEXTRW` (legacy, opcode 0xC4/0xC5, 66-mandatory-prefix, GAS's own `pinsrw $imm8, gpr32/m16, xmm`/`pextrw $imm8, xmm, gpr32` syntax) and their VEX siblings `vpinsrw`/`vpextrw` (commit `ef8a4d7`) - the first family combining two things this session had built separately but never combined: cross-register-class (GPR<->XMM) operands and a trailing imm8 selector. `pinsrw`/`vpinsrw` reuse the existing immediate-carrying representations (`Lowered.Sse_binop_imm_r_rm`/`Lowered.Vex_binop_imm_rr_rm`) unchanged, since the only real difference from `SHUFPS`/`VSHUFPS`'s own use of those shapes is that one operand's register class is GPR rather than xmm - the ModR/M/SIB/imm8-byte-emission machinery does not care which register class occupies a field, only `reg_field`/`rm_of`'s `~width` parameter needs to select 32 (GPR) instead of 128 (xmm) for that one operand, mirroring `Cvtsi2f_r_rm`'s own cross-class precedent. `pextrw`/`vpextrw` reuse `Lowered.Sse_binop_imm_r_rm`/`Lowered.Vex_unop_imm_r_rm` the same way, and - since XED's own `PEXTRW_GPR32_XMMdq_IMMb` (SSE2) and `VPEXTRW_GPR32d_XMMdq_IMMb_C5` (AVX) records share the exact same three-operand shape - one new normalizer (`pextrw_rr_form`) is dispatched for both the legacy and VEX iforms verbatim, the same reuse `movd_load_rr_form`/`movd_store_rr_form` already established for `vmovd`. No REX.W-equivalent variant exists for either mnemonic (always a 32-bit GPR): confirmed against real GNU as, `pinsrw $1,%rax,%xmm0` assembles identically to the `%eax` spelling (`66 0f c4 c0 01`, no REX.W emitted either way), so `width_ok`'s exact-32-bit-match naturally rejects the 64-bit-named spelling as an unimplemented-but-real GNU as substitution, the same deliberate boundary `Vmovd`'s own doc comment already documents for `vmovq`. `PEXTRW`'s own memory-destination restriction was confirmed, not assumed: real GNU as routes `pextrw $1,%xmm0,(%eax)` to the unrelated, three-byte-opcode (`0F3A 15`) SSE4.1 `PEXTRW_SSE4_MEMw_XMMdq_IMMb` instruction instead (`66 0f 3a 15 00 01`) - this project has no `0F38`/`0F3A` opcode-map infrastructure at all (confirmed absent when `PACKSSWB` was admitted), so that spelling is out of scope, and `vpextrw $1,%xmm1,(%eax)` was independently confirmed to make the identical substitution under VEX (`c4 e3 79 15 08 01`, three-byte VEX). `VPEXTRW`'s own register-register form also has a confirmed, deliberately-not-taken redundant alternate: XED's export carries both `VPEXTRW_GPR32d_XMMdq_IMMb_C5` and `VPEXTRW_GPR32d_XMMdq_IMMb_15`, two real GNU-as-reachable encodings for the same instruction, but real GNU as's own register-register spelling always selects the `_C5`-suffixed one - the same "low-numbered iform" precedent `Movapd` already established - so only `_C5` is admitted. All 12 new records (6 forms x 2 profiles) confirmed byte-for-byte against real GNU as before writing any code, and the fixed corpus-registration gap from the immediately preceding correction was applied here too - `Isa_gen_difficult`'s case-entry lists were extended in the same commit as the admission, not left for a future fork to discover missing, and confirmed present with real bytes in the regenerated `cases.jsonl` by direct grep, not just accounting counts (matching the discipline the preceding correction itself named as the fix). Validation: `tools-test`, `tools-integration` (2 stale pinned counts corrected: `xed_resolved` normalized 488->494/490->496, `xed_resolved` promoted-support 477->483/blocked 7399->7393 (x86-32) and promoted-support 485->491/blocked 10081->10075 (x86-64), and the `isa-norm-jsonl` round-trip count 2474->2486, all re-derived fresh via the CLI, not guessed), `tools-boundary`, `asm-isa-difficult-regen`+`check` (all 12 new cases pass byte-for-byte; the two legacy forms show the same pre-existing "different_observed_form" opcode-byte diagnostic quirk every other mandatory-66-prefixed legacy family already has, not a regression), full `asm-test` (one cram golden-file promotion for the four new codec-table rows), `asm-fmt-check` (one reflow pass) - all pass. SSE2 now at promoted-support 153/263 (x86-32) / 159/268 (x86-64); AVX now 192/676 (x86-32) / 192/702 (x86-64). GEN-05 now also admits `PUNPCKLBW`/`PUNPCKHBW`/`PUNPCKLWD`/`PUNPCKHWD`/`PUNPCKLDQ`/`PUNPCKHDQ` (packed integer interleave at byte/word/dword lane widths, opcodes 0x60/0x68/0x61/0x69/0x62/0x6A, 66-mandatory-prefix only - `PUNPCKLQDQ`'s own group at narrower lane widths, no non-66 sibling since these are integer SIMD) and their VEX siblings `vpunpcklbw`/`vpunpckhbw`/`vpunpcklwd`/`vpunpckhwd`/`vpunpckldq`/`vpunpckhdq`, both register-register and register<-memory, both x86-32/x86-64 - 24 new records per profile (12 legacy + 12 VEX) - reusing `Sse_binop_r_rm`/`xmm_binop_rr_form`/`xmm_binop_rm_form` (legacy) and `Vex_binop_rr_rm`/`vex_binop_rrr_form`/`vex_binop_rr_mem_form` (VEX) completely unchanged, pure opcode-table extension into the existing 66-mandatory-prefix legacy table and `pp=1` VEX table, zero new representation in either arc. Confirmed byte-for-byte against real GNU as (i686-linux-gnu-as/x86_64-linux-gnu-as 2.44) before writing any code (e.g. `punpcklbw %xmm2,%xmm1` -> `66 0f 60 ca`, `vpunpcklbw %xmm3,%xmm2,%xmm1` -> `c5 e9 60 cb`). Validated via `tools-test`, `tools-integration` (pinned counts re-derived fresh via the CLI: `xed_resolved` normalized 494->518/496->520, promoted-support 483->507/blocked 7393->7369 (x86-32) and promoted-support 491->515/blocked 10075->10051 (x86-64), and the `isa-norm-jsonl` round-trip count 2486->2534), `tools-boundary`, `asm-isa-difficult-regen`+`check` (all 24 new cases pass byte-for-byte; confirmed present in `cases.jsonl` directly, not just accounting, after this session's earlier `PMULLW`-family case-coverage gap), full `asm-test` (one cram golden-file promotion for the grown `sse-binop-66-op`/`vex-scalar-66-op` codec tables), `asm-fmt-check` (one reflow pass) - all pass. SSE2 now at promoted-support 165/263 (x86-32) / 171/268 (x86-64); AVX now 204/676 (x86-32) / 204/702 (x86-64). |
