...
let test_counts () =
  ...
  check "all includes every difficult-form family"
    ...
    = List.length Isa_gen_difficult.sw_entries
      ...
      + List.length Isa_gen_difficult.x86_xmm_shift_imm_entries
  ...
  check "x86_sse_binop_imm_rr_entries has 28 entries (14 mnemonics, one per target)"
    (List.length Isa_gen_difficult.x86_sse_binop_imm_rr_entries = 28);
  check "x86_sse_binop_imm_rm_entries has 28 entries (14 mnemonics, one per target)"
    (List.length Isa_gen_difficult.x86_sse_binop_imm_rm_entries = 28);
