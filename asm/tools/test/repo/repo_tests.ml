...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:7544;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:10181
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:138 ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized_only:0
    ~gas_generatable:5 ~promoted_support:7248 ~blocked:496;
  expect ~oracle_unavailable:116 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:5 ~promoted_support:9571 ~blocked:879
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 19938)"
       !roundtrip_count)
    (!roundtrip_count = 19938)
