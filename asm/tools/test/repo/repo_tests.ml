...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:6953;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:7158
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:137 ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized_only:0
    ~gas_generatable:5 ~promoted_support:6704 ~blocked:1041;
  expect ~oracle_unavailable:115 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:5 ~promoted_support:6911 ~blocked:3540
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 16324)"
       !roundtrip_count)
    (!roundtrip_count = 16324)
