...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:7644;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:10351
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:153 ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized_only:0
    ~gas_generatable:5 ~promoted_support:7347 ~blocked:382;
  expect ~oracle_unavailable:159 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:5 ~promoted_support:9725 ~blocked:682
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 20208)"
       !roundtrip_count)
    (!roundtrip_count = 20208)
