...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:518;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:520
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 2534)"
       !roundtrip_count)
    (!roundtrip_count = 2534)
