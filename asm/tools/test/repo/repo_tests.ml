...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:1017;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:1019
...
let test_isa_family_admission repo =
  ...
  (* Promotes the four bare scalar single-precision arithmetic forms
     ...
     PSHUFD/PSHUFLW/PSHUFHW (opcode 0x70, 66/F2/F3 mandatory-prefix groups,
     GEN-05) reuse `Sse_binop_imm_r_rm`/`xmm_binop_imm_rr_form`/
     ...
     equal, 0x64/0x65/0x66 greater-than, GEN-05) reuse the same shapes as
     ...
     PAND/PANDN/POR (opcodes 0xDB/0xDF/0xEB, GEN-05) reuse the same shapes as
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 3532)"
       !roundtrip_count)
    (!roundtrip_count = 3532)
