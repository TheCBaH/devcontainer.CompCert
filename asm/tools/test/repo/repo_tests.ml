...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:7291;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:8964
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:137 ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized_only:0
    ~gas_generatable:5 ~promoted_support:7027 ~blocked:718;
  expect ~oracle_unavailable:115 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:5 ~promoted_support:8573 ~blocked:1878
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 18468)"
       !roundtrip_count)
    (!roundtrip_count = 18468)
