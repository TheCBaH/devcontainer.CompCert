...
let test_isa_norm_accounting repo =
  ...
  in
  expect ~source:"riscv_opcodes" Target.Riscv32 ~total:1089 ~normalized:701;
  expect ~source:"riscv_opcodes" Target.Riscv64 ~total:1154 ~normalized:753;
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 1472)"
       !roundtrip_count)
    (!roundtrip_count = 1472)
