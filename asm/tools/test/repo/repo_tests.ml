...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:608;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:610
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 2714)"
       !roundtrip_count)
    (!roundtrip_count = 2714)
