...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:7629;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:10335
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:138 ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized_only:0
    ~gas_generatable:5 ~promoted_support:7332 ~blocked:412;
  expect ~oracle_unavailable:141 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:5 ~promoted_support:9710 ~blocked:715
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 20177)"
       !roundtrip_count)
    (!roundtrip_count = 20177)
