...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized:5752;
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:5823
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:136 ~source:"xed_resolved" Target.X86_32 ~total:7887 ~normalized_only:0
    ~gas_generatable:3 ~promoted_support:4522 ~blocked:3226;
  expect ~oracle_unavailable:114 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:3 ~promoted_support:4590 ~blocked:5864
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 13788)"
       !roundtrip_count)
    (!roundtrip_count = 13788)
