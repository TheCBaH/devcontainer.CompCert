...
let test_isa_norm_accounting repo =
  ...
  expect ~source:"xed_resolved" Target.X86_64 ~total:10571 ~normalized:7512
...
let test_isa_family_admission repo =
  ...
  expect ~oracle_unavailable:115 ~source:"xed_resolved" Target.X86_64 ~total:10571
    ~normalized_only:0 ~gas_generatable:5 ~promoted_support:7258 ~blocked:3193
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 17016)"
       !roundtrip_count)
    (!roundtrip_count = 17016)
