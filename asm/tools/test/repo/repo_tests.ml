...
let test_isa_norm_accounting repo =
  ...
  in
  expect ~source:"riscv_opcodes" Target.Riscv32 ~total:1089 ~normalized:700;
  expect ~source:"riscv_opcodes" Target.Riscv64 ~total:1154 ~normalized:752;
...
let test_isa_norm_jsonl_roundtrip repo =
  ...
  check
    (Printf.sprintf "isa-norm-jsonl: %d real normalized forms round-tripped (expected 1470)"
       !roundtrip_count)
    (!roundtrip_count = 1470)
