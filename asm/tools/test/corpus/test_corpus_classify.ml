...
let test_classify_all () =
  ...
  | Ok [ { gas = Some (Gas_ok { objdump_sha256 }); _ } ] ->
      ...
      [
        {
          ...
          generated_sha256 = Some (String.make 64 'b');
          outcome = Rec_accepted;
          gas = Some (Gas_ok { objdump_sha256 = String.make 64 '1' });
        };
        {
          path = "modules/CompCert/test/c/fib.c";
          source_sha256 = String.make 64 'c';
          generated_sha256 = Some (String.make 64 'd');
          outcome = Rec_rejected "unsupported directive .foo";
          gas = Some (Gas_error "Error: no such instruction");
        };
      ];
  }
  ...
  check "parse_manifest: a '..'-escaping path is rejected"
    (is_err
       (parse_manifest
         ...
         ^ "\tgenerated-sha256:" ^ String.make 64 'b' ^ "\toutcome:accepted" ^ good_gas ^ "\n")));
  check "parse_manifest: a well-formed compile-failed record (no gas field) is accepted"
    (is_ok
       (parse_manifest
          (good_header ^ "modules/CompCert/test/c/broken.c\tsource-sha256:" ^ String.make 64 'a'
         ^ "\toutcome:compile-failed\treason:x\n")));
