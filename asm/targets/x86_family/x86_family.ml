...
module Make (M : MODE) = struct
  ...
  let split_rip (slice : Asm_core.Token.slice) =
  ...
  let disp_expression ~bad (slice : Asm_core.Token.slice) =
    match Asm_core.Parse_lines.parse_expression slice with
    | Error _ ->
        bad (Printf.sprintf "cannot parse displacement %s" (Asm_core.Token.slice_text slice))
    | Ok e -> (
  ...
  let parse_one_operand (slice : Asm_core.Token.slice) =
    ...
    | _ -> (
        ...
        | None -> (
            ...
            | Error _ ->
                bad (Printf.sprintf "cannot parse operand %s" (Asm_core.Token.slice_text slice))))
    ...
    let depth_of slice =
      List.fold_left
        (fun d t ->
          ...
          | Asm_core.Token.Lparen -> d + 1
          | Asm_core.Token.Rparen -> d - 1
          | _ -> d)
        0 slice
    in
