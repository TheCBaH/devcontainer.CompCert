...
module Opcode = struct
  type t =
    ...
    | Paddb
        (** [paddb rm, reg] - packed integer add, byte lanes ([66 0F FC /r], GEN-05),
    ...
    | Psubq
        (** [psubq rm, reg] - packed integer subtract, quadword lanes ([66 0F FB /r]), {!Psubb}'s
            sibling. *)
    | Pcmpeqb
        (** [pcmpeqb rm, reg] - packed compare-equal, byte lanes ([66 0F 74 /r], GEN-05),
    ...
    | Pcmpgtd
        (** [pcmpgtd rm, reg] - packed compare-greater-than, doubleword lanes ([66 0F 66 /r]),
            {!Pcmpgtb}'s sibling. *)
    | Packsswb
        (** [packsswb rm, reg] - pack words into bytes with signed saturation ([66 0F 63 /r],
            GEN-05), {!Paddb}'s own 66-mandatory-prefix-only integer-SIMD group at a different
    ...
    | Packuswb
        (** [packuswb rm, reg] - pack words into bytes with unsigned saturation ([66 0F 67 /r]),
            {!Packsswb}'s sibling. *)
    | Pand
        (** [pand rm, reg] - packed bitwise AND, integer ([66 0F DB /r], GEN-05), {!Pxor}'s own
    ...
    | Pminub
        (** [pminub rm, reg] - packed integer minimum, unsigned byte lanes ([66 0F DA /r],
            GEN-05), {!Paddb}'s own 66-mandatory-prefix-only integer-SIMD group at a different
    ...
    | Pshufd
        (** [pshufd $imm8, rm, reg] - packed shuffle doublewords ([66 0F 70 /r ib], GEN-05):
  ...
  let pp ppf i =
    ...
    | ops -> (
        ...
        | ( Opcode.Addsd | Opcode.Subsd | Opcode.Mulsd | Opcode.Divsd | Opcode.Addss | Opcode.Subss
          ...
          | Opcode.Unpcklpd | Opcode.Unpckhpd | Opcode.Punpcklqdq | Opcode.Punpckhqdq | Opcode.Paddb
          ...
          | Opcode.Pmaxsw | Opcode.Andps | Opcode.Andnps | Opcode.Orps | Opcode.Xorps | Opcode.Andpd
          | Opcode.Andnpd | Opcode.Orpd | Opcode.Movaps | Opcode.Movups | Opcode.Movupd
          ...
          | Opcode.Maxps | Opcode.Minps | Opcode.Maxpd | Opcode.Minpd | Opcode.Sqrtss
          ...
          | Opcode.Vpunpcklqdq | Opcode.Vpunpckhqdq | Opcode.Vpaddb | Opcode.Vpaddw | Opcode.Vpaddd
          | Opcode.Vpaddq | Opcode.Vpsubb | Opcode.Vpsubw | Opcode.Vpsubd | Opcode.Vpsubq
          | Opcode.Vpcmpeqb | Opcode.Vpcmpeqw | Opcode.Vpcmpeqd | Opcode.Vpcmpgtb | Opcode.Vpcmpgtw
          ...
          | Opcode.Vpmaxsw | Opcode.Vmovd | Opcode.Fldl | Opcode.Fstpl | Opcode.Fstps | Opcode.Flds
...
module Make (M : MODE) = struct
  ...
  let simplify_instruction ~features s =
    ...
    (* {!Opcode.Unpcklps}'s integer-SIMD sibling (GEN-05): 66-mandatory-prefix only, same
    ...
    (* {!Opcode.Paddb}'s own doc comment (GEN-05): packed integer add/subtract, 66-mandatory-
    ...
    (* {!Opcode.Pcmpeqb}'s own doc comment (GEN-05): packed compare-equal/greater-than,
    ...
    (* {!Opcode.Packsswb}'s own doc comment (GEN-05): pack-with-saturation, {!Opcode.Paddb}'s own
    ...
    (* {!Opcode.Pand}'s own doc comment (GEN-05): packed integer bitwise AND/AND-NOT/OR,
    ...
    (* {!Opcode.Pminub}'s own doc comment (GEN-05): packed integer min/max, {!Opcode.Paddb}'s own
       group at different opcode bytes. *)
    ...
    (* {!Opcode.Movapd}'s data-movement siblings: aligned/unaligned packed move, single/double
       precision, load direction only (GEN-05, matching {!Opcode.Movapd}'s own precedent). *)
    ...
    (* [shufps]/[shufpd] (GEN-05): the first XMM-immediate-carrying legacy shape, [imm, rm, reg]
    ...
    (* [pshufd]/[pshuflw]/[pshufhw] (GEN-05): {!Opcode.Pshufd}'s own doc comment - the unary
    ...
    (* {!Opcode.Vpand}'s own doc comment (GEN-05): the VEX sibling of the legacy
    ...
    (* {!Opcode.Vaddsd}/{!Opcode.Vandps}'s min/max siblings (GEN-05): the VEX counterpart of the
    ...
    (* {!Opcode.Vsqrtsd}'s own doc comment (GEN-05): the VEX sibling of the legacy
    ...
    (* {!Opcode.Vpshufd}'s own doc comment (GEN-05): the VEX sibling of the legacy
  ...
  let lower_instruction state i =
    ...
    | Opcode.Shld, [ Operand.Imm v; Operand.Reg src; Operand.Reg dst ] -> (
        ...
        | Ok imm -> (
            ...
            | Error e, _ | _, Error e -> Error e))
    (* [shufps $imm8, rm, reg]/[shufpd $imm8, rm, reg] (GEN-05): {!Opcode.Shld}'s own
    ...
    | ( ( Opcode.Addsd | Opcode.Subsd | Opcode.Mulsd | Opcode.Divsd | Opcode.Addss | Opcode.Subss
        ...
        | Opcode.Punpckhqdq | Opcode.Paddb | Opcode.Paddw | Opcode.Paddd | Opcode.Paddq
        ...
        | Opcode.Por | Opcode.Pminub | Opcode.Pmaxub | Opcode.Pminsw | Opcode.Pmaxsw ),
    ...
    | ( ( Opcode.Vaddsd | Opcode.Vsubsd | Opcode.Vmulsd | Opcode.Vdivsd | Opcode.Vaddss
        ...
        | Opcode.Vpunpckhqdq | Opcode.Vpaddb | Opcode.Vpaddw | Opcode.Vpaddd | Opcode.Vpaddq
        | Opcode.Vpsubb | Opcode.Vpsubw | Opcode.Vpsubd | Opcode.Vpsubq | Opcode.Vpcmpeqb
        | Opcode.Vpcmpeqw | Opcode.Vpcmpeqd | Opcode.Vpcmpgtb | Opcode.Vpcmpgtw | Opcode.Vpcmpgtd
        | Opcode.Vpacksswb | Opcode.Vpackssdw | Opcode.Vpackuswb | Opcode.Vpand | Opcode.Vpandn
        | Opcode.Vpor | Opcode.Vpminub | Opcode.Vpmaxub | Opcode.Vpminsw | Opcode.Vpmaxsw
    ...
    | ( ( Opcode.Vshufps | Opcode.Vshufpd | Opcode.Vcmpss | Opcode.Vcmpsd | Opcode.Vcmpps
        ...
        | Ok imm -> (
            ...
            | Error e, _ | _, Error e -> Error e))
    (* [vpshufd $imm8, src, dst]/[vpshuflw $imm8, src, dst]/[vpshufhw $imm8, src, dst] (GEN-05,
       {!Opcode.Vpshufd}'s own doc comment): {!Vex_unop_r_rm}'s own [src, dst] operand order,
  ...
  (* The mandatory-prefix-free members of the binop family - {!Comiss}, and
     GEN-05's packed-single logical siblings {!Andps}/{!Andnps}/{!Orps}/
  ...
  (* {!vex_unop_alt}'s trailing-immediate sibling for {!Lowered.Vex_unop_imm_r_rm}
     ([vpshufd]/[vpshuflw]/[vpshufhw] only, GEN-05): no [vvvv] operand, like {!vex_unop_alt},
  ...
  let codec : (Lowered.t, fixup_kind) C.t =
    C.choice ~name:M.name
      ...
      @ [
          ...
          (* [vpshuflw]/[vpshufhw]/[vpshufd] (GEN-05): {!Vpshuflw}/{!Vpshufhw}'s own one-entry
