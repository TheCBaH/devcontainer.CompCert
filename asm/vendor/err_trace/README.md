...
record of *which* version is vendored is the submodule's gitlink — which is
exactly what a submodule is for. Unlike [`../fmt`](../fmt/README.md), there is
no SHA-256 table to keep in step: `git submodule status` already answers that
question.
...
`upstream/` is a whole upstream repository, not just the two files we build.
...
[`./dune`](./dune) marks `upstream/` `data_only_dirs`, which leaves the files
reachable as ordinary sources while reading no dune file inside it.
`tools/asm-check-planted.sh` has a case that deletes that line and requires the
